        ...
        while cont==0
            ...
            for i=1:le
                if ~constFACT(i)
                    ...
                    % Reduce problem size for mode 2:end
                     if i == 1
                        Xm=X1;
                    elseif i == 2
                        Xm=FACT{1}'*X1;
                        Xm=unmatrizicing(Xm,1,[noc, sizeX(2:end)]);
                        Xm=matrizicing(Xm,i);
                     else
                        Xm=unmatrizicing(Xm,i-1,[noc, sizeX(2:end)]);
                        Xm=matrizicing(Xm,i);
                     end
                    
                     % Calculate covariances and matricized factor loadings
                    if ind(1)==1
                        kr = sparse(1:noc,1:noc,ampl,noc,noc);
                        krkr = FACT{k(1)}'*FACT{k(1)};             
                    else
                        kr = FACT{ind(1)};
                        krkr = FACT{k(1)}'*FACT{k(1)});             
                    end
                    ...
                    elseif lambda(i)==0
                        if normmeth==0                           
                           tx = sum(Wy.*FACT{i},1);
                           ty = sum(Wx.*FACT{i},1);
                           Wxt = Wx + repmat(tx,[N(i),1]).*FACT{i};
                           Wyt = Wy + repmat(ty,[N(i),1]).*FACT{i};
                           T=(Wxt+eps)./(Wyt+eps);                   
                        else
            ...
            sse=SST-2*sum(sum(Wx.*FACT{i}))+sum(sum(krkr.*(FACT{i}'*FACT{i})));
