...
%               Thus, measure=[0 0 1 0 0 0 2]  Calculates the AmpCorr
...
%               cop(4) 1: Use time point 0: do not use time point.
%               cop(5) 1: Use time frequency point 0: do not use frequency point.
%               default: cop=[0 0 0 0]
%
% Output:
...
%
% Revision:
% 30 November 2006      Amplitude Correlation measure implemented
...
if size(WT,4)>1
    ...
    elseif measure(3)==1    
        ...
        Y=0;
        sWT=size(WT);
        if cop(4) & cop(5)  %Use time frequency point
                if measure(7)==1 % ERLCOH
                    Y=sum(repmat(conj(WT(ch,f,t,:)),[sWT(1:3) 1]).*WT,4);
                    N1=repmat(sum(abs(WT(ch,f,t,:)).^2,4),[sWT(1:3)]);
                    N2=sum(abs(WT).^2,4);
                elseif measure(7)==2 % AmpCorr
                    WT=abs(WT);
                    WT=WT-repmat(mean(WT,4),[1 1 1 size(WT,4)]);
                    WT=WT./sqrt(1/sWT(4)*repmat(sum(WT.^2,4),[1 1 1 sWT(4)]));
                    Y=mean(repmat(WT(ch,f,t,:),[sWT(1:3) 1]).*WT,4);
                else    % ERPCOH
                    Y=mean(repmat(conj(WT(ch,f,t,:))./abs(WT(ch,f,t,:)),[sWT(1:3) 1]).*(WT./abs(WT)),4);
                end
        elseif cop(4) % Use time point
                if measure(7)==1 % ERLCOH
                    Y=sum(repmat(conj(WT(ch,:,t,:)),[sWT(1) 1 sWT(3) 1]).*WT,4);
                    N1=repmat(sum(abs(WT(ch,:,t,:)).^2,4),[sWT(1) 1 sWT(3)]);
                    N2=sum(abs(WT).^2,4);
                elseif measure(7)==2 % AmpCorr
                    WT=abs(WT);
                    WT=WT-repmat(mean(WT,4),[1 1 1 size(WT,4)]);
                    WT=WT./sqrt(1/sWT(4)*repmat(sum(WT.^2,4),[1 1 1 sWT(4)]));
                    Y=mean(repmat(WT(ch,:,t,:),[sWT(1) 1 sWT(3) 1]).*WT,4);
                else    % ERPCOH
                    Y=mean(repmat(conj(WT(ch,:,t,:))./abs(WT(ch,:,t,:)),[sWT(1) 1 sWT(3) 1]).*(WT./abs(WT)),4);
                end
        elseif cop(5) % Use frequency point
                if measure(7)==1 % ERLCOH
                    Y=sum(repmat(conj(WT(ch,f,:,:)),[sWT(1:2) 1 1]).*WT,4);
                    N1=repmat(sum(abs(WT(ch,f,:,:)).^2,4),[sWT(1:2) 1]);
                    N2=sum(abs(WT).^2,4);
                elseif measure(7)==2 % AmpCorr
                    WT=abs(WT);
                    WT=WT-repmat(mean(WT,4),[1 1 1 size(WT,4)]); 
                    WT=WT./sqrt(1/sWT(4)*repmat(sum(WT.^2,4),[1 1 1 sWT(4)]));
                    Y=mean(repmat(WT(ch,f,:,:),[sWT(1:2) 1 1]).*WT,4);
                else    % ERPCOH
                   Y=mean(repmat(conj(WT(ch,f,:,:))./abs(WT(ch,f,:,:)),[sWT(1:2) 1 1]).*(WT./abs(WT)),4);
                end
            
        else    % Only use channel
                if measure(7)==1 %ERLCOH
