...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    %Need to rename "top1err" to "error" before adding to the stats structure...
    %[net,stats.train(epoch),prof] = process_epoch(net, state, opts, 'train') ;
    [net, tempTrain, prof] = process_epoch(net, state, opts, 'train');    
    tempTrain.error = tempTrain.top1err;
    tempTrain = rmfield(tempTrain, 'top1err');
    stats.train(epoch) = tempTrain;
    
    %[~,stats.val(epoch)] = process_epoch(net, state, opts, 'val') ;
    [~,tempVal] = process_epoch(net, state, opts, 'val') ;
    tempVal.error = tempVal.top1err;
    tempVal = rmfield(tempVal, 'top1err');
    stats.val(epoch) = tempVal;
    if opts.profile
      profview(0,prof) ;
      keyboard ;
    end
