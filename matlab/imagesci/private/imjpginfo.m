...
assert(isequal(sig, [255; 216]),'Not a JPEG file.');


[info,exif_offset, idx] = imjpgbaselineinfo(fid);
fclose(fid);
...
raw_tags = tifftagsread(filename, offset, 0, 1);

exif = tifftagsprocess ( raw_tags );
if isempty(exif)
    return;
end
...
if numel(idx) > 1
    raw_tags = tifftagsread(filename, offset, 1, 1);
    thumbnail = tifftagsprocess(raw_tags);
else
    thumbnail = [];
end
