...
%   HDFGD is not recommended.  Use MATLAB.IO.HDFEOS.GD instead.
%
%   HDFGD is the MATLAB interface to the HDF-EOS Grid object. HDF-EOS is an
%   extension of NCSA (National Center for Supercomputing Applications) HDF
...
%       Creates a grid data set. GRIDNAME is a string contain the name of
%       the grid data set. XDIMSIZE and YDIMSIZE are scalars indicating the
%       sizes of respective dimensions. UPLEFT is a two-element vector
%       containing the X and Y coordinates of the upper left pixel of the
%       grid. LOWRIGHT is a two-element vector containing the X and Y
%       coordinates of the lower right pixel of the grid.  GRID_ID is -1 if
%       the operation fails.
%   
%   GDattach
%       GRID_ID = HDFGD('attach',FILE_ID,GRIDNAME)
%       Attaches to an existing grid data set within the file. GRID_ID is
...
%       Defines which corner of the grid is the origin. ORIGIN can be 'ul',
%       'ur', 'll', or 'lr'.
%
%   GDdefdim
%       STATUS = HDFGD('defdim',GRID_ID,DIMNAME,DIM)
%       Defines a new dimension within the grid data set. DIMNAME is a
%       string containing the name of the new dimension, and DIM is a
%       scalar containing the size of the new dimension.
%
%   GDdefproj
%       STATUS = HDFGD('defproj',GRID_ID,PROJCODE,...
%                 ZONECODE,SPHERECODE,PROJPARM)
%       Defines the map projection for a grid data set. PROJCODE can be
%       'geo', 'utm', 'lamcc', 'ps', 'polyc', 'tm', 'lamaz', 'snsoid', 
%       'hom', 'som', 'good','isinus', 'cea' or 'bcea'. ZONECODE is the 
%       Universal Transverse Mercator zone code; specify [] or 0 for other
%       projections. SPHERECODE is a scalar spheroid code. PROJPARM is a
%       vector of up to 13 elements containing projection-specific
%       parameters. For more details about PROJCODE, ZONECODE, SPHERECODE,
%       and PROJPARM, see Chapter 6 of HDF-EOS Library Users Guide for the
%       ECS Project, Volume 1: Overview and Examples.
%
%   GDdefpixreg
%       STATUS = HDFGD('defpixreg',GRID_ID,PIXREG)
%       Defines pixel registration within grid cell.  PIXREG can be
%       'center' or 'corner'.
%
%   GDdeffield
%       STATUS = HDFGD('deffield',GRID_ID,FIELDNAME,...
%                         DIMLIST,NUMBERTYPE,MERGE)
%       Defines data fields to be stored in a grid.  FIELDNAME is a string
%       containing the name of the dimension to be defined.  DIMLIST is a
%       string containing a comma-separated list of dimension names.
%       NUMBERTYPE is a string specifying the HDF number type.  MERGE can
...
%       COMPCODE can be 'rle', 'skphuff', 'deflate', or 'none'.  Deflate
%       compression requires that COMPPARM be an integer from one to nine,
%       with higher values corresponding to higher compression. Specify 0
%       or [] for COMPPARM for other compression methods.
%
%   GDwritefieldmeta
%       STATUS = HDFGD('writefieldmeta',GRID_ID,...
%                   FIELDNAME,DIMLIST,NUMBERTYPE)
%       Writes field metadata for an existing grid field that was not
%       defined using the Grid API.  FIELDNAME is a string containing the
%       name of the field.  DIMLIST is a string containing a
%       comma-separated list of dimension names. NUMBERTYPE a string
%       specifying the HDF number type.
%
%   GDblkSOMoffset
%	    STATUS = HDFGD('blksomoffset',GRID_ID,OFFSET)
%	    Writes the block SOM offset values, in pixels, from
%       a standard SOM projection. OFFSET is a vector of offset values for
%       SOM projection data.  This routine can only be used with grids that
%       use the SOM projection.
%
%   GDsettilecomp
...
%       values.  This routine was introduced in HDF-EOS 2.5 to fix a bug in
%       HDF-EOS 2.3.  FIELDNAME is the name of the specified grid.
%       TILEDIMS is a vector containing the tile dimensions. COMPCODE can
%       be 'rle', 'skphuff', 'deflate', or 'none'. Deflate compression
...
%       Writes data to the specified field of a grid data set.  FIELDNAME
%       is the name of the specified grid. START is an vector specifying
%       the starting location (zero-based) within each dimension.  STRIDE
%       is a vector specifying the number of values to skip along each
%       dimension.  EDGE is the number of values to write along each
%       dimension.  DATA is the array of values to be written.  Specifying
%       [] for START is the same as specifying ZEROS(1,NUMDIMS).
%       Specifying [] for STRIDE is the same as specifying ONES(1,NUMDIMS).
%       Specifying [] for EDGE is the same as specifying
%       FLIPLR(SIZE(DATA)).
%
...
%       dimension sizes of the HDF data field.  For example, if the grid
...
%       Reads data from specified grid field.  FIELDNAME is the name of the
%       field to read.  START is a vector specifying the starting location
%       (zero-based) within each dimension.  STRIDE is a vector specifying
%       the number of values to skip along each dimension.  EDGE is a
%       vector containing the number of values to read along each
%       dimension.  Specifying [] for START is the same as specifying
%       ZEROS(1,NUMDIMS).  Specifying [] for STRIDE is the same as
%       specifying ONES(1,NUMDIMS). Specifying [] for EDGE implies that the
%       entire field will be read.
%
%       NOTE: HDF files use C-style ordering for multidimensional arrays,
%       while MATLAB uses FORTRAN-style ordering.  This means that the size
%       of the MATLAB array must be flipped relative to the defined
%       dimension sizes of the HDF data field.  For example, if the grid
%       field has been defined to have dimensions 3-by-4-by-5, then DATA
%       will have size 5-by-4-by-3.  The PERMUTE command is useful for
%       making any necessary conversions.
%
...
%       Writes or updates the grid data set attribute with the specified
%       name. If the attribute does not already exist, it is created.
%   
%   GDreadattr
%       [DATA,STATUS] = HDFGD('readattr',GRID_ID,ATTRNAME)
%       Reads the attribute data from the specified attribute.  DATA is []
%       and STATUS is -1 if the operation fails.
%   
%   GDsetfillvalue
%       STATUS = HDFGD('setfillvalue',GRID_ID,...
%                                FIELDNAME,FILLVALUE)
%       Sets the fill value for the specified field. FILLVALUE is a scalar
...
%                            FIELDNAME)
%       Retrieves the fill value for the specified field. FILLVALUE is []
%       and STATUS is -1 if the operation fails.
%
%   GDij2ll
%       [LON,LAT,STATUS] = HDFGD('ij2ll',GRID_ID,ROW,COL);
%       Converts the grid's (i,j) coordinates to longitude and latitude in
%       decimal degrees.  STATUS is -1 if the operation fails.
%       
%   GDll2ij
%       [ROW,COL,X,Y,STATUS] = HDFGD('ll2ij',GRID_ID,LON,LAT);
%       Converts longitude and latitude (in decimal degrees) to ROW and COL
%       (i,j) coordinates in a grid.  In addition, the X and Y positions
%       (scaled distances) of the point in the grid are returned.  STATUS 
%       is -1 if the operation fails.
%
%   GDrs2ll
%       [LON,LAT,STATUS] = HDFGD('rs2ll',GRID_ID,R,S);
%       Converts EASE grid's (r,s) coordinates to longitude and latitude 
%       in decimal degrees.  The grid must have a BCEA projection.  STATUS 
%       is -1 if the operation fails.
%
%   Inquiry routines
%   ----------------
...
%       Retrieves information about all of the dimensions defined in a grid
%       data set.  NDIMS is the number of dimensions. DIMNAME is a string
%       containing a comma-separated list of dimension names. DIMS is a
%       vector containing the size of each dimension.  NDIMS is -1 and
%       DIMNAME and DIMS are [] if the operation fails.
%
%   GDinqfields
%       [NFIELDS,FIELDLIST,RANK,NUMBERTYPE] = HDFGD( ...
%                'inqfields', GRID_ID)
%       Retrieves information about all of the fields defined in a grid
%       data set.  NFIELDS is the number of fields. FIELDLIST is a string
%       containing a comma-separated list of field names.  RANK is a vector
%       containing the rank of each field.  NUMBERTYPE is a cell array of
%       strings containing the HDF number type of each field.
%
%   GDinqattrs
%       [NATTRS,ATTRLIST] = HDFGD('inqattrs',GRID_ID)
%       Retrieves information about all of the attributes defined in a
%       grid. NATTRS is the number of attributes. ATTRLIST is a string
%       containing a comma-separated list of attribute names. NATTRS is -1
%       and ATTRLIST is [] if the operation fails.
%
%   GDnentries
%       [NENTRIES,STRBUFSIZE] = HDFGD('nentries',GRID_ID,...
%                 ENTRYCODE)
%       Returns the number of entries for a specified entity. ENTRYCODE can
%       be either 'nentdim' to query the number of dimensions, or
%       'nentdfld' to query the number of fields. STRBUFSIZE is the length
%       of the comma-separated list of dimension or field names.
%
%   GDgridinfo
%       [XDIMSIZE,YDIMSIZE,UPLEFT,LOWRIGHT,STATUS] = ...
%                 HDFGD('gridinfo',GRID_ID)
%       Returns position and size of grid data set. XDIMSIZE and YDIMSIZE
%       are scalars containing the respective dimension size. UPLEFT is a
%       two-element vector containing the X and Y coordinates of the
%       upper-left grid corner. LOWRIGHT is a two-element vector containing
%       the X and Y coordinates of the lower-right grid corner. STATUS is
%       -1 and all other outputs are [] if the operation fails.
%
%   GDprojinfo
%       [PROJCODE,ZONECODE,SPHERECODE,PROJPARM,STATUS] = ...
%                 HDFGD('projinfo',GRID_ID)
%       Retrieves projection information about a grid data set.  See
%       GDdefproj above for a description of the output parameters.  STATUS
%       is -1 and all other outputs are [] if the operation fails.
%
%   GDdiminfo
%       DIMSIZE = HDFGD('diminfo',GRID_ID,DIMNAME)
%       Retrieves the size of the specified dimension. DIMNAME is a string
%       containing the dimension name. DIMSIZE is -1 if the operation
%       fails.
%
%   GDcompinfo
%       [COMPCODE,COMPPARM,STATUS] = HDFGD('compinfo',...
%                          GRID_ID,FIELDNAME)
%       Retrieves compression information about the specified field.  See
%       GDdefcomp above for a description of the output parameters.  STATUS
%       is -1 and the other outputs are [] if the operation fails.
%
%   GDfieldinfo
%       [RANK,DIMS,NUMBERTYPE,DIMLIST,STATUS] = ...
%             HDFGD('fieldinfo',GRID_ID,FIELDNAME)
%       Retrieves information about a specified field. RANK is the field
%       rank. DIMS is a vector containing the dimension sizes of the field.
%       NUMBERTYPE is a string containing the HDF number type of the field.
%       DIMLIST is a string containing a comma-separated list of dimension
%       names.  STATUS is -1 and all other outputs are [] if the operation
%       fails.
%
%   GDinqgrid
%       [NGRID,GRIDLIST] = HDFGD('inqgrid',FILENAME)
%       Retrieves information about the grid data sets in an HDF-EOS file.
%       NGRID is the number of grids, and GRIDLIST is a string containing a
%       comma-separated list of grid data set names.  NGRID is -1 and
%       GRIDLIST is [] if the operation fails.
%
%   GDattrinfo
...
%       string corresponding to the HDF number type of the attribute.
%       COUNT is the number of bytes used by the attribute data.  STATUS is
%       -1 and NUMBERTYPE and COUNT are [] if the operation fails.
%
%   GDorigininfo
%       ORIGINCODE = HDFGD('origininfo',GRID_ID)
%       Returns the origin code for a grid data set. ORIGINCODE is 'ul',
%       'ur', 'll', or 'lr'.
%
%   GDpixreginfo
%       PIXREGCODE = HDFGD('pixreginfo',GRID_ID)
%       Returns the pixreg code for a grid data set. PIXREGCODE is either
%       'center' or 'corner'.
%
%   Subset routines
%   ---------------
...
%                            CORNERLON,CORNERLAT)
%       Defines a longitude-latitude box region for a grid data set.
%       CORNERLON is a two-element vector containing the longitudes of
%       opposite box corners.  CORNERLAT is a two-element vector containing
%       the latitudes of opposite box corners.  REGION_ID is -1 if the
%       operation fails.
%   
%   GDregioninfo
...
%                              REGION_ID,FIELDNAME)
%       Returns information about the subsetted region. NUMBERTYPE is the
%       HDF number type of the field. RANK is the rank of the field. DIMS
...
%   HDFGD is not recommended.  Use MATLAB.IO.HDFEOS.GD instead.
%
%   See also MATLAB.IO.HDFEOS.GD, MATLAB.IO.HDFEOS.SW, HDF, HDFPT.
