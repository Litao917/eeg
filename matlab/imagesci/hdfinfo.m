...
if strcmpi(mode,'hdf')
    ...
    if sdID==-1
        warning(message('MATLAB:imagesci:hdfinfo:interfaceStart','SD'));
    end
...
elseif strcmpi(mode,'eos')
    ...
    if gdID==-1
        warning(message('MATLAB:imagesci:hdfinfo:interfaceStart','GD'));
    end
    swID = hdfsw('open',filename,'read');
    if swID==-1
        warning(message('MATLAB:imagesci:hdfinfo:interfaceStart','SW'));
    end
...
if ~strcmpi(mode,'eos')
    if sdID ~= -1
        [~,nGlobalAttrs,status] = hdfsd('fileinfo',sdID);
        hdfwarn(status)
        if nGlobalAttrs>0
            for i=1:nGlobalAttrs
                [fileAttribute(i).Name,~,~,status] = hdfsd('attrinfo',sdID,i-1);
                hdfwarn(status)
...
[ndatasets,~, status] = hdfsd('fileinfo',sdID);
hdfwarn(status)
for i=1:ndatasets
...
if numSwaths>0
    ...
    swinfo = hdfswathinfo(filename,fileID,swathList{1});
    swinfo = repmat(swinfo,1,numSwaths);
    for i=2:numSwaths
        swinfo(i) = hdfswathinfo(filename,fileID,swathList{i});
    end
end
return;
...
if numGrids>0
    ...
    gdinfo = hdfgridinfo(filename,fileID,gridList{1});
    gdinfo = repmat(gdinfo,1,numGrids);
    for i=2:numGrids
        gdinfo(i) = hdfgridinfo(filename,fileID,gridList{i});
    end
end
return;
