...
%   HDFSD is not recommended.  Use MATLAB.IO.HDF4.SD instead.
%
%   HDFSD is a gateway to the HDF multifile scientific dataset interface (SD). 
%
%   The general syntax for HDFSD is HDFSD(funcstr,param1,param2,...). There 
%   is a one-to-one correspondence between SD functions in the HDF library 
%   and valid values for funcstr.  For example, HDFSD('endaccess',sds_id) 
%   corresponds to the C library call SDendaccess(sds_id).
%
%   Syntax conventions
%   ------------------
...
%   failed.
%
%   A tf output is either 1 for true or 0 for false.
%
%   SD_id refers to the multifile scientific dataset interface identifier.
%   sds_id refers to an individual dataset identifier. You must be sure to
%   terminate access to all opened identifiers using either
%   hdfsd('end',SD_id) or hdfsd('endaccess',sds_id); otherwise the HDF
%   library may not properly write all data to the file. 
%   
%   HDF files use C-style ordering for multidimensional arrays, while
%   MATLAB uses FORTRAN-style ordering.  This means that the size of the
%   MATLAB array must be flipped relative to the defined dimension sizes of
%   the HDF data set.  For example, if the HDF data set has dimensions
%   3-by-4-by-5, then the equivalent MATLAB array has size 5-by-4-by-3.
%   The PERMUTE command is useful for making any necessary conversions when
%   reading from or writing to HDF data sets.
%
%   In cases where the HDF C library accepts NULL for certain inputs, an
%   empty matrix ([]) can be used.
%
%   Access functions
%   ----------------
...
%       Returns the identifier of the data set with the specified index.
%
%     SD_id = hdfsd('start',filename,access_mode)
%       Initializes the SD interface for a particular file; access_mode can
%       be 'read', 'write', 'create', 'rdwr', or 'rdonly'.
%
...
%       Reads data from a chunked or nonchunked data set NOTE: the
%       coordinates in the vector start should be zero-based, not
%       one-based.
%
%     status = hdfsd('setexternalfile', sds_id, filename, offset)
%       Sets up an external file to store a data set.  The data starts at
%       the specified byte offset in the external file.
%
%     status = hdfsd('writedata',sds_id, start, stride, edge, data)
%       Writes data to a chunked or nonchunked data set NOTE: The class of
%       data must match the HDF number type defined for the HDF data set.
%       A MATLAB string will be automatically converted to match any of the
%       HDF char types; other data types must match exactly. NOTE: the
%       coordinates in the vector start should be zero-based, not
%       one-based.
%
%   General inquiry functions
%   -------------------------
%   General inquiry functions return information about the location,
%   contents, and description of the scientific data sets in an HDF file.
%
...
%
%     tf = hdfsd('isrecord',sds_id)
%       Determines if a dataset is appendable.
%
%     idx = hdfsd('nametoindex',SD_id,name)
%       Returns index value of the named data set.
%
%     sds_index = hdfsd('reftoindex',SD_id,ref)
%       Returns the index of the data set corresponding to a given
%       reference number.
%
%   Dimension scale functions
%   -------------------------
%   Dimension scale functions define and access dimension scales within a
%   data set.
%
...
%
%     tf = hdfsd('isdimval_bwcomp',dim_id)
%       Determines if a dimension is backwards compatible with versions
%       of HDF prior to 4.1r1.
%
...
%
%     status = hdfsd('setdimval_comp',dim_id,mode)
%       Defines the backwards-compatibility of a dimension with versions of
%       HDF prior to 4.1r1.  mode can be one of the following strings:
%
%           'comp'   - specifies backwards compatible mode
%           'incomp' - specifies backwards incompatible mode, the default
%
%   User-defined attribute functions
%   --------------------------------
%   User-defined attribute functions describe and access characteristics of
%   an HDF file, data set, or dimension defined by the HDF user.
%
%     [name,data_type,count,status] = hdfsd('attrinfo',id,attr_idx)
%       Gets information about an attribute; id can be an SD_id, an sds_id,
%       or a dim_id.
%     
%     attr_index = hdfsd('findattr',id,name)
%       Returns the index of the specified attribute; id can be an SD_id, 
%       an sds_id, or a dim_id.
%
%     [data,status] = hdfsd('readattr',id,attr_index)
%       Reads the values of the specified attribute; id can be an SD_id, an
%       sds_id, or a dim_id.
%
%     status = hdfsd('setattr',id,name,A)
%       Creates and defines a new attribute; id can be an SD_id, an sds_id,
%       or a dim_id.
%
...
%       Returns the label, limit, format, and coordinate system of a data
%       set.
%
...
%
%     previous_fillmode = hdfsd('setfillmode',SD_id,mode)
%       Defines the fill mode to be applied to all scientific data sets in
%       the specified file.  
%
...
%   ---------------------
%   Compression functions determine the compression method for scientific
%   data sets.
%
...
%   -------------------------
%   Chunking/tiling functions determine the chunking configuration for SDS
%   data.
%
%     [chunk_lengths,chunked,compressed,status] = hdfsd('getchunkinfo',sds_id)
%       Obtains information about a chunked scientific data set; chunked or
%       compressed is true (1) if the SDS is chunked or compressed.
%
%     maxcache_out = hdfsd('setchunkcache',sds_id,maxcache,flags)
%       Sets the size of the chunk cache.  Upon success, maxcache_out is 
...
%   HDFSD is not recommended.  Use MATLAB.IO.HDF4.SD instead.
%
%   See also MATLAB.IO.HDF4.SD, HDF, HDFAN, HDFDF24, HDFDFR8, HDFH, HDFHD, 
