...
switch sel_type
    case 'normal' % left click
        ...
        % present
        if ~graphicsversion(ax(1),'handlegraphics')
            for i = 1:numel(ax)
                hax=ax(i);
                if isprop(hax,'LayoutManager') && isvalid(hax.LayoutManager);
                    lm = hax.LayoutManager;
                    lm.enableAxesDirtyListeners(false); 
                end
            end
        end
...
cmd.Name = getString(message('MATLAB:uistring:pan:Pan'));
...
if ~isempty(ax) && ishghandle(ax(1))
    ...
    if ~graphicsversion(ax(1),'handlegraphics')
        for i = 1:numel(ax)
            ...
            if isprop(hax,'LayoutManager') && isvalid(hax.LayoutManager);
                lm = hax.LayoutManager;
                lm.enableAxesDirtyListeners(true); 
            end
        end
    end
end
...
if ~graphicsversion(fig,'handlegraphics')
    ...
    curr_units = hgconvertunits(fig,[0 0 evd.(propName)],...
...
if strcmp(hMode.ModeStateData.mouse,'off')
    set(obj,'CurrentPoint',curr_units);
    hAx = locFindAxes(obj,evd);
...
if ~graphicsversion(fig,'handlegraphics')
    propName = 'Point';
    curr_pixel = hgconvertunits(fig,[0 0 evd.(propName)],...
       get(fig,'Units'),'pixels',fig);
    curr_pixel = curr_pixel(3:4);
else
    propName = 'CurrentPoint';
    curr_pixel = evd.(propName);
end
...
if hMode.ModeStateData.is2D
    for i = 1:length(axVector)
        ...
        if ~all(isfinite(curr_lim1)) || ~all(isfinite(curr_lim2)) ...
            ...
            [curr_lim1, curr_lim2] = matlab.graphics.interaction.internal.getFiniteLimits(ax);
        ...
        % Consider direction of axis: [{'normal'|'reverse'}] or negative
        % log scale
        dir1 = get(ax,sprintf('%sdir',abscissa(1)));
        if strcmp(dir1,'reverse') || is_abscissa_neg_log
        ...
        if strcmp(dir2,'reverse') || is_ordinate_neg_log
        ...
        if is_abscissa_log
            new_lim1 = real(10.^new_lim1);
            curr_lim2 = real(10.^curr_lim2); %#ok
        end
        if is_ordinate_log
            curr_lim1 = real(10.^curr_lim1); %#ok
            new_lim2 = real(10.^new_lim2);
        end
...
props.Callback = '';
props.Label = getString(message('MATLAB:uistring:pan:PanOptions'));
...
props.Label = getString(message('MATLAB:uistring:pan:HorizontalPanAppliesTo2DPlotsOnly'));
props.Tag = 'PanHorizontal';
uimenu(props);

props.Label = getString(message('MATLAB:uistring:pan:VerticalPanAppliesTo2DPlotsOnly'));
