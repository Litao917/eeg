...
case 2
    ...
    if ischar(varargin{1}) && strcmp(varargin{1},'newzoom')
        localSetNewZoom(varargin{2});
        return;
    else
        fig=varargin{1};
        if ~any(ishghandle(fig))
           error(message('MATLAB:v6_zoom:InvalidArgument'))
        end
...
if (nargout ~= 0) && ...
    ...
    error(message('MATLAB:v6_zoom:LimitedOutputReturn'));
...
otherwise
    error(message('MATLAB:v6_zoom:UnknownOption', zoomCommand));
end
