...
for i=1:length(axeshandles)
    ...
    elseif isempty(legend(axeshandles(i))) && (isempty(legH) || (~isempty(legH) && axesH~=axeshandles(i)))
        continue;
    end
    ...
    createaxislegend(axeshandles(i),axesdata,legH);
end
...
% get legend info
% Check for MCOS Graphics
if graphicsversion(axesH,'handlegraphics') == 1
    % Non-MCOS
...
% Check for MCOS Graphics
if graphicsversion(dataH,'handlegraphics') ~= 1
    % MCOS
    H = handle(H);
end