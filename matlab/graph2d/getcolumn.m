...
if nargin == 2
    ...
    if ~isempty(token)
        ...
        elseif strcmp(expr,'displayname')
            % Special Case for DisplayName lookup
            exprBefore = '';
        end
    elseif strcmp(expr,'displayname')
        % Special Case for DisplayName lookup
    ...
    if length(n) == 1
        % If this is a DisplayName and we could not find a variable in the
        % workspace then just return the display name string
        if strcmp(expr,'displayname') && isempty(exprBefore)
            y = x;
        else
        ...
        for k=1:length(n)
            if strcmp(expr,'displayname') && isempty(exprBefore)
                % If this is a DisplayName and we could not find a variable
                % in the workspace then just return the display name
                % appended with the column number
                y{k} = [x,num2str(k)];
            else
