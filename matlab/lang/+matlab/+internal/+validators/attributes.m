...
for i=1:numAttrs
    if(ischar(attrs{i}) && strcmp(attrs{i},'scalar'))
...
while idx <= numAttrs
    ...
    if isempty(attributor) 
        ...
        if warnOnUnknownAttr
            if ischar( attr )
                warning(message('MATLAB:validateattributes:attributeNotFound', attr ))
            else
...
if isnan(A) || ~isreal(A) || ~all( fcn(diff(A),0) )
    argDes = matlab.internal.validators.getArgumentDescriptor( msgId, argname, argpos );
...
% and throw the error
if ~any(cellfun(@(c)isa(A,c), classes))
