...
%         if iscolon(rowIndices) && isequal(sizeLHS,size(b_data{j}))) && isa(b_data{j},class(var_j))
%             var_j = b_data{j};
%         else
            % Do this as a subscripted assignment even when rowIndices is ':',
            % because RHS may not have same type or trailing size
            try %#ok<ALIGN>
                if isa(var_j,'table')
                    var_j = subsasgnParens(var_j,{rowIndices ':'},b_data{j}(:,:)); % can't use table subscripting directly
                else
                    var_j(rowIndices,:) = b_data{j}(:,:);
                end
