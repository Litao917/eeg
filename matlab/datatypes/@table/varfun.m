...
%
%   See also TABLE/ROWFUN, CELLFUN, STRUCTFUN, ARRAYFUN.
...
if grouped
    ...
    for jvar = 1:ndataVars
        for igrp = 1:ngroups
            rows = (group == igrp);
            inArg = getVarRows(a_data{dataVars(jvar)},rows);
            try
    ...
    elseif tableOutput
        ...
        try
            b = [subsrefParens(a,{repelem(grpRowLoc,grpNumRows),groupVars}) ...
                 table(grpCounts(repelem(1:ngroups,grpNumRows)),'VariableNames',vnames(1)) ...
