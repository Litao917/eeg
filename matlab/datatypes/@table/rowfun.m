...
%
%   See also TABLE/VARFUN, CELLFUN, STRUCTFUN, ARRAYFUN.
...
if grouped
    ...
    n = length(grpNames);
    grpCounts = histc(group,1:n); % ignores NaNs in group
else
    n = a.nrows;
end
...
for i = 1:n
    ...
    if separateArgs
        inArgs = extractRows(a,rows,extractCells); inArgs = inArgs(dataVars);
        try
            if nout > 0
                [b_data{i,:}] = fun(inArgs{:});
            else
        ...
        inArgs = subsrefBraces(a,{rows dataVars}); % inArgs = a{rows,dataVars}
        try
            if nout > 0
                [b_data{i,:}] = fun(inArgs);
            else
...
elseif tableOutput
    if nout > 0
        ...
        if grouped
            ...
            bc = table(grpCounts(repelem(1:n,grpNumRows)),'VariableNames',{'GroupCount'});
            if ~supplied.OutputVariableNames
            ...
            b = [bg bc b];
            b.rownames = matlab.lang.makeUniqueStrings(repelem(grpNames,grpNumRows),{},namelengthmax);
        else
