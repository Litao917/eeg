...
%   table element.
%
%   See also TABLE, @TABLE/DISPLAY, FORMAT.
...
if matlab.internal.display.isHot() && bold
    strongBegin = '<strong>';strongEnd = '</strong>'; 
end
...
if (t.nrows > 0) && (t.nvars > 0)
    ...
    if isempty(t.rownames)
        ...
        maxWidth = maxWidth + length([strongBegin strongEnd]);
    ...
    for ivar = 1:t.nvars
        ...
        % first variable, otherwise we'd display only the row names.
        if ivar > 1 && size(tblChars,2) + between + size(varChars,2) > maxWidth
            dispVarNames((ivarOld+1):(ivar-1));
            disp(tblChars);
            fprintf(['\n' looseline]);
            if isempty(t.rownames)
                tblChars = indentSpaces;
