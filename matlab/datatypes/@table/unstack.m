...
%      flu2(iflu2,:)
%
%   See also TABLE/STACK, TABLE/JOIN.
...
for t = 1:ntallVars
    ...
    if isnumeric(tallVar) || islogical(tallVar) % but not char
        ...
        for k = 1:ncols
            tallVar_k = tallVar(~missing,k); % leave out rows with missing groupingindicator
            if isempty(fun)
    ...
    % Handle non-numeric types indirectly
        ...
        if isempty(fun)
            ...
            wideRowIndices = accumarray({jdx,kdx},(1:nrowsTall)',[nrowsWide,nwideVars]);
            for j = 1:nwideVars
            ...
            wideRowIndices = accumarray({jdx,kdx},(1:nrowsTall)',[nrowsWide,nwideVars],@(x) {x});
            for j = 1:nwideVars
                % Create the wideVar with the same class as tallVar.
