...
if isempty(obj)
    ...
    if (StructL > 1) || (strcmp(Struct(1).type,'()')==0)
        error(message('MATLAB:timer:unhandledsyntax'));
    end
        ...
        if strcmp(Struct(lcv).type,'()')
            if isa(val{lcv},'timer') % if timers, work with jobject
                ...
                elseif ~isa(val{lcv+1},'timer')
                    error(message('MATLAB:timer:assigntonontimerobject'));
                ...
                elseif length(val{lcv+1}.jobject) == 1 || length(Struct(lcv).subs{:}) == length(val{lcv+1}.jobject) ...
                    ...
                    error(message('MATLAB:timer:assignelementsizemismatch'));
                end                    
            else
                error(message('MATLAB:timer:inconsistentsubscript'));
            end
...
if (length(val{1}) == 0)
    ...
    error(message('MATLAB:timer:emptytimerarray'));
else
...
for i=1:subsL
    if ~(islogical(subs{i}) || isnumeric(subs{i}) || ischar(subs{i}))
        error(message('MATLAB:timer:badsubscript', class(subs{i})));
    end
end

if subsL == 1 % single dim. reference, e.g., t(1)
    if (~islogical(subs{1}) && (min(subs{1}) < 1)) % e.g., t(0)
        error(message('MATLAB:timer:nonpositiveindex'));
    end
    if isnumeric(subs{1}) && max(subs{1}) > length(obj)+1 % e.g., t(5), where length(t) = 2
        ...
        if ~(all(diff(subs{1}) == 1) && (subs{1}(1) == (length(obj) + 1)))
            error(message('MATLAB:timer:gapsnotallowed'));
        end
    end
else % e.g., t(1,2)   
    sz = zeros(subsL,1);
