...
% Copyright 2002-2013 The MathWorks, Inc.
    ...
    methods
        ...
        %    See also TIMER/SET, TIMER/TIMERFIND, TIMER/START, TIMER/STARTAT.
            ...
            mlock
            if ((nargin>0 && all(ishandle(varargin{1})) && all(isJavaTimer(varargin{1}))))
                % this flavor of the constructor is not intended to be for the end-user
                % TIMER.EMPTY method utilizes this code path
                if ~isvector(varargin{1}) % not a vector, sorry.
                    error(message('MATLAB:timer:creatematrix'));
                end
                obj.jobject = varargin{1}; % make a MATLAB timer object from one or more java timer object(s)
    ...
    methods ( Static=true, Hidden=true ) 
        ...
        function obj = empty(varargin)
            if nargin > 0
                ...
                if ~isequal(indices, [1 0]) && ~isequal(indices, [0 0])
                    error(message('MATLAB:timer:empty'))
                end
            end
            tempObj = timer;
            subs =  {{zeros(1,0)}};
        ...
        %Designed method for wrapping JAVA timer objects in MATLAB Handle. 
        %Intended only for internal use by TIMERFIND.M
        function obj = wrapJavaTimerObjs(jobjects)
            obj = timer.empty;
            obj.jobject = jobjects;
        end
    end
