...
jobjs = jobjs(2:end); % first found jobjs is the database, so remove it
jobjs(arrayfun(@isInvisibleTimer,jobjs,'UniformOutput',true)) = [];
...
function TF=isInvisibleTimer(jobj)
...
try
    TF=isfield(get(jobj),'ObjectVisibility') && strcmp(jobj.ObjectVisibility,'off');
catch
    TF=true;
end
end