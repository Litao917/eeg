...
%   $Revision: 1.8.4.7 $  $Date: 2011/09/13 22:37:02 $
...
if (numRead ~= totalSize) 
    ...
    if (nargout ~= 3)
        ...
        % Display warning.
        warning(message('MATLAB:serial:fread:unsuccessfulRead', warnMsg));
...
if (nargout ~= 3) && ~isempty(varargout{3})
    ...
    warning(message('MATLAB:serial:fread:unsuccessfulRead', varargout{ 3 }));
