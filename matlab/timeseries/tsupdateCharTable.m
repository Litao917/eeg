function tsupdateCharTable(h,additionalDataProps,addedArgs,charlist1,model,startRow,~)
%
...
ind = find(strcmp(char(model.getValueAt(startRow,1)),cellfun(@(x) x,...
    charlist(:,1),'UniformOutput',false)));
if isempty(ind)
    return
end
...
%% Parse additional char data props
adddataprop = cell(length(additionalDataProps),1);
...
if ~isempty(additionalDataProps) && any(cellfun('isempty',adddataprop))
