...
if tsrownum>=0
    ...
    if col==0 % Time vector modification
        ...
        if strcmp(recorder.Recording,'on')
            ...
            if reSortFlag
               T.addbuffer(sprintf('time = %s.Time;',genvarname(thisTs.Name)));
               T.addbuffer(sprintf('time(%d) = %f;',tsrownum,time(tsrownum)));
               T.addbuffer('[time,I] = sort(time);');
               T.addbuffer(sprintf('%s.Data = %s.Data(I,:);',genvarname(thisTs.Name),...
                   genvarname(thisTs.Name)));
               if ~isempty(thisTs.Quality)
                   T.addbuffer(sprintf('%s.Quality = %s.Quality(I);',genvarname(thisTs.Name),...
                       genvarname(thisTs.Name)));
               end
               T.addbuffer(sprintf('%s.Time = time;',genvarname(thisTs.Name)),h.Timeseries);
            else
               T.addbuffer(sprintf('%s.Time(%d) = %s;',genvarname(thisTs.Name),tsrownum,...
                   newValue),thisTs); 
            end
        end
    elseif col<=size(thisTs.Data,2) % Data column
        ...
        if strcmp(recorder.Recording,'on')
            T.addbuffer(sprintf('%s.Data(%d,%d) = %s;',genvarname(thisTs.Name),tsrownum,col,newValue),thisTs); 
        end
