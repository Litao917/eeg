...
try
    ...
    if qualFlag
        ...
        if strcmp(recorder.Recording,'on')
            T.addbuffer(sprintf('%% Add a new row to timeseries ''%s''',genvarname(h.Timeseries.Name)));
            T.addbuffer(sprintf('%s = addsample(%s, ''Time'',[%s], ''Data'',[%s], ''Quality'',[%s], ''OverwriteFlag'',true);',...
                genvarname(h.Timeseries.Name), genvarname(h.Timeseries.Name), num2str(time), num2str(data(:).'),...
        ...
        if strcmp(recorder.Recording,'on')
             ...
             T.addbuffer(sprintf('%s = addsample(%s, ''Time'',[%s], ''Data'',[%s], ''OverwriteFlag'',true);',...
                    genvarname(h.Timeseries.Name), genvarname(h.Timeseries.Name), num2str(time), num2str(data(:)')),h.Timeseries);
        end
    end
