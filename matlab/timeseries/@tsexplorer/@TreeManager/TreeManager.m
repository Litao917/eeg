...
% if ~ismac && ~feature('HGUsingMATLABClasses')
...
javaMethodEDT('setCursor',this.Tree.getTree,java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR))
...
javaMethodEDT('setHorizontalScrollBarPolicy',this.Tree.getScrollPane,32);
...
javaMethodEDT('setDropTarget',this.Tree.getTree,dropHandler);
...
function localMarginDown(~,~,this,pos)
...
[~,this.DragMargin{pos}] = javacomponent(thisButton,[0 0 1 1],this.Figure);    
set(this.DragMargin{pos},'Parent',this.Figure,'Units',get(this.Margin(pos),'Units'),...
    'Position',DragMarginPos);


function localMarginUp(~,ed,this,pos)
...
function localSlideMargin(~,~,this,pos)
...
function localHelpVisCallback(~,~,h)
