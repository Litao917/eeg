...
%   of the datametadata object H to the value VALUE.  An equivalent syntax 
%   is 
...
%
%   See also DATAMETADATA\GET.

%   Copyright 2006-2011 The MathWorks, Inc.

ni = nargin;
no = nargout;
if ~isa(h,'tsdata.datametadata'),
   % Call built-in SET. Handles calls like set(gcf,'user',ss)
   builtin('set',h,varargin{:});
   return
elseif no && ni>2,
   error(message('MATLAB:tsdata:datametadata:set:invOutArgs'));
end
...
elseif ni==2,
   ...
   ename = inputname(1);
   if isempty(ename),
      error(message('MATLAB:tsdata:datametadata:set:invVarArg'))
   elseif rem(ni-1,2)~=0,
      error(message('MATLAB:tsdata:datametadata:set:invPropValPairs'))
   end
