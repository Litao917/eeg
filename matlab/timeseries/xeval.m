...
%% that subsumed time series names are not artificially detected
...
for k=1:length(tsnames)
    ...
    if ~isempty(pos) && sum(strcmp(tsnames{k},tsnames))>=2
        error(message('MATLAB:xeval:duplicateTSName'));
    elseif ~isempty(pos)
       % Create a local variable containing
       data = tslist{k}.Data;
       ...
       % names are not falsely detected within longer time series names
       I = [];
       for j=1:length(pos)
