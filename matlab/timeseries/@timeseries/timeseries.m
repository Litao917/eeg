...
classdef (CaseInsensitiveProperties = true) timeseries
    ...
    methods (Static = true)
        ...
        function h = loadobj(s)
         ...
         elseif isa(s,'timeseries')
             ...
             if ~isempty(h.Data_)
                 if (h.IsTimeFirst_ ~= tsdata.datametadata.isTimeFirst(size(h.Data_),h.Length,h.DataInfo.InterpretSingleRowDataAs3D))
                    s = size(h.Data_);
                    if ~ismatrix(h.Data_)
                        if (h.Version < 10.2)
    ...
    methods (Static = true, Hidden = true)
      ...
      function [data,quality] = utreshape(len,data,quality)
            ...
            if (ndims(data) >=3 && len > 1 && len~=size(data,ndims(data)))
                error(message('MATLAB:timeseries:utreshape:datatimearraymismatch'));
            end
