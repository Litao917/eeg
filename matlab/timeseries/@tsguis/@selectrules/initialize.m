...
h.Figure = figure('Units','Characters','Position',[104 31.2989 100 31],'Toolbar','None',...
    'Menubar','None','NumberTitle','off','Name',getString(message('MATLAB:timeseries:tsguis:selectrules:SelectDataUsingRules')),...
    'Visible','off','closeRequestFcn',{@localClose h},...
    'HandleVisibility','callback','IntegerHandle','off');

%% Build buttons
h.Handles.BTNok = uicontrol('Style','Pushbutton','String', ...
    getString(message('MATLAB:timeseries:tsguis:selectrules:OK')), ...
    'Parent',h.Figure,'Units','Characters',...
    'Callback',{@localOK h},'BusyAction','Cancel','Interruptible','off');
h.Handles.BTNapply = uicontrol('Style','Pushbutton','String', ...
    getString(message('MATLAB:timeseries:tsguis:selectrules:Apply')), ...
    'Parent',h.Figure,'Units','Characters',...
    'callback',{@localApply h},'BusyAction','Cancel','Interruptible','off');
h.Handles.BTNcancel = uicontrol('String', ...
    getString(message('MATLAB:timeseries:tsguis:selectrules:Cancel')), ...
    'Parent',h.Figure,'Units','Characters',...
    'Callback',{@localClose h});
h.Handles.BTNhelp = uicontrol('String', ...
    getString(message('MATLAB:timeseries:tsguis:selectrules:Help')), ...
    'Parent',h.Figure,'Units','Characters','Callback',{@localDispatchhelp h});
...
set(h.Handles.TSPanel,'Units','Characters',...
    'Title', ...
    getString(message('MATLAB:timeseries:tsguis:selectrules:SelectTheTimePlotAndTimeSeries')));
h.Handles.LBLselts = uicontrol('style','text','String', ...
    getString(message('MATLAB:timeseries:tsguis:selectrules:SelectTheTimePlot')), ...
    'Units',...
...
h.Handles.tsTableModel = tsMatlabCallbackTableModel(cell(0,4),...
            {getString(message('MATLAB:timeseries:tsguis:selectrules:SelectFromyn')), ...
            getString(message('MATLAB:timeseries:tsguis:selectrules:TimeSeries')), ...
            getString(message('MATLAB:timeseries:tsguis:selectrules:Path')), ...
            getString(message('MATLAB:timeseries:tsguis:selectrules:SelectedColumns'))},...
            [],[]);
...
try % Prevent command line errors
    ...
    for k=1:size(tsTableData,1)
        if tsTableData{k,1}
            ...
            if ~isempty(thists) && ~isempty(thesecols) && all(thesecols>=1) && ...
                ...
                errordlg(getString(message('MATLAB:timeseries:tsguis:selectrules:InvalidColumnsEnteredForTimeSeries', ...
                    thists.Name)),...
                    getString(message('MATLAB:timeseries:tsguis:selectrules:TimeSeriesTools')), ...
                    'modal')
    ...
    %% Exit normalization mode so that the selected curves will display
    %% correctly
        ...
        if strcmp(recorder.Recording,'on') 
            if ~isequal(selectedCols{k},1:size(tsList{k}.Data,2))
             h.ViewNode.Plot.SelectionStruct.History = [h.ViewNode.Plot.SelectionStruct.History;...
                History;... 
               {['%% ' getString(message('MATLAB:timeseries:tsguis:selectrules:RestrictingRuleSelectionToSpecifiedColumns'))]};...
               {[ 'I' genvarname(tsList{k}.Name) ' = false(size(' genvarname(tsList{k}.Name) '.Data));' ]};... 
               {[ 'I' genvarname(tsList{k}.Name) '(:,[' num2str(selectedCols{k}) ']) = true;' ]};...
               {['%% ' getString(message('MATLAB:timeseries:tsguis:selectrules:ApplyingCombinedSelectionRules'))]};...
               {[ 'I' genvarname(tsList{k}.Name) '= I' genvarname(tsList{k}.Name) ' & idx;' ]}];
            else
              h.ViewNode.Plot.SelectionStruct.History = [h.ViewNode.Plot.SelectionStruct.History;...
                                                         History;...
                                                         {[ 'I' genvarname(tsList{k}.Name) '= idx;' ]}];               
            end
        end  
    ...
    if nargin<=3 &&  emptyset
        drawnow
        warndlg(getString(message('MATLAB:timeseries:tsguis:selectrules:TheCombinationOfTheSelectionRules')),...
    ...
    if strcmp(type,'time') && ~isempty(h.ViewNode.Plot) && ...
         ...
         if ~isempty(topdate)
             try
                 ...
                 errordlg(getString(message('MATLAB:timeseries:tsguis:selectrules:TimeBoundsMustBeValidDateStrings')),...
                     getString(message('MATLAB:timeseries:tsguis:selectrules:TimeSeriesTools')), ...
                     'modal')
         ...
         if ~isempty(bottomdate)
             try
                 ...
                 errordlg(getString(message('MATLAB:timeseries:tsguis:selectrules:TimeBoundsMustBeValidDateStrings')),...
                     getString(message('MATLAB:timeseries:tsguis:selectrules:TimeSeriesTools')),'modal')
...
try %#ok<TRYNC>
    ...
    if isempty(flatlinelen) || ~isscalar(flatlinelen) || ~isfinite(flatlinelen) || ...
        ...
        uiwait(errordlg( ...
            getString(message('MATLAB:timeseries:tsguis:selectrules:LengthMustBeAFiniteInteger')), ...
            getString(message('MATLAB:timeseries:tsguis:selectrules:TimeSeriesTools')), ...
            'modal')) 
...
%% Evaluate expression with x =1 to check that it is valid and returns a
%% logical
try %#ok<TRYNC> % Prevent command line errors
    x = 1; %#ok<NASGU>
    ...
    %% If valid, change the process rule, else reset
    if ~isempty(outchk) && islogical(outchk)    
        set(h,'Mexpression',char(TXTExp.getText))
    else
        uiwait(errordlg( ...
            getString(message('MATLAB:timeseries:tsguis:selectrules:ExpressionMustEvaluateToALogicalArray')), ...
            getString(message('MATLAB:timeseries:tsguis:selectrules:TimeSeriesTools')), ...
            'modal'))
        set(h,'Mexpression','')
...
if strcmp(h.ViewNode.Plot.Absolutetime,'off')
    errordlg(...
        getString(message('MATLAB:timeseries:tsguis:selectrules:DateStringsCanOnlyBeUsedForPlotsThatUseCalendarDates')),...
        getString(message('MATLAB:timeseries:tsguis:selectrules:TimeSeriesTools')), ...
        'modal')
    return
end
...
if isempty(h.Calendar)
    h.Calendar = Calendar(TXTTime, ...
        getString(message('MATLAB:timeseries:tsguis:selectrules:SpecifyDateTime')), ...
        TXTTime);
else
...
tsDispatchHelp('d_select_data_on_plot','modal',h.Figure)