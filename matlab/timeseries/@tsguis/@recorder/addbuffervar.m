...
%% to be used in constructing the code for the buffer

filepath = fullfile(h.Path,h.Filename);
matfilepath = sprintf('%s.mat',filepath);

%% If there is no MAT file or no data has been previosly saved
%% - create one, write the index variable into it with variable name I1
if ~exist(matfilepath) || strcmp(h.Saveddata,'off') %#ok<EXIST>
