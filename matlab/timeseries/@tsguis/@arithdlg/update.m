...
for k=1:length(tsNames)
    ...
    if length(tsnodes)==1 && ...
        ...
        % after aliases are substituted into the expression strings 
...
%% Populate the table
h.Handles.tsTable.getModel.setDataVector(tableData,{getString(message('MATLAB:tsguis:arithdlg:update:Identifier')),getString(message('MATLAB:tsguis:arithdlg:update:Name')),getString(message('MATLAB:tsguis:arithdlg:update:Path')),getString(message('MATLAB:tsguis:arithdlg:update:Size'))},...
    h.Handles.tsTable);
