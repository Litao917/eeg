...
h.Handles.InitTXT = uicontrol('style','text','string',...
    getString(message('MATLAB:timeseries:tsguis:viewnode:DragAndDropToView')), ...
    'BackgroundColor', [.4 .4 .4],'ForegroundColor',[1 1 1],...
    'Parent',f);
...
%% Add node name
if nargin == 3 
  h.Label = getString(message('MATLAB:timeseries:tsguis:viewnode:ViewNode'));
elseif nargin == 4
  h.Label = varargin{1};
elseif nargin >= 5 
  h.Label = h.createDefaultName( varargin{1}, varargin{2} );
else
  error(message('MATLAB:tsguis:viewnode:initialize:noNode'))
end
...
%% Figure closing listener detaches view node. The node detachment
%% listener then disposes of the view

%% Select the view parent. This ensures that the New Plot panels on
...
%% Node detachment listener. Sets plot and view invisible and destroys the
%% figure
...
% Remove any drop adapters
h.DropAdaptor.targetTree = [];
% Destroy the plot first so that linked axes callbacks don't fire
p = [];
