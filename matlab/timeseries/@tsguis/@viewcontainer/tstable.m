...
if nargin>=2 && ~isempty(varargin{1}) && ~isempty(varargin{1}.Plot) &&...
    ...
    tableData = cell([1 3]); % Must be at least 1 row or col format will be lost
end

%% Populate the table - if necessary creating it
headings = { ...
    getString(message('MATLAB:timeseries:tsguis:viewcontainer:TimeSeries')), ...
    getString(message('MATLAB:timeseries:tsguis:viewcontainer:Path')), ...
    getString(message('MATLAB:timeseries:tsguis:viewcontainer:Dimensions'))}; 
