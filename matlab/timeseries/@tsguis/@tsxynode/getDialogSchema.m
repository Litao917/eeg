...
h.Handles.PNLTs = uipanel('Parent',f,'Units','Characters','Title', ...
    getString(message('MATLAB:timeseries:tsguis:tsxynode:DefineDisplayedTimeSeries')),'Visible','off');
h.Handles.BTNEditView = uicontrol('Style','pushbutton','Parent', ...
    f,'Units','Characters', ...
    'String',getString(message('MATLAB:timeseries:tsguis:tsxynode:EditPlot')),...
...
if ~isempty(this.Plot) && ishandle(this.Plot) 
    ...
    errordlg(getString(message('MATLAB:timeseries:tsguis:tsxynode:PlotIsEmpty')),...
        getString(message('MATLAB:timeseries:tsguis:tsxynode:TimeSeriesTools')),'modal')
end
