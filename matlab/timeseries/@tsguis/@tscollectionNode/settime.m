...
if nargin>=5 %% Format
    ...
    if strcmp(recorder.Recording,'on')
        T.addbuffer([genvarname(h.Tscollection.Name), '.TimeInfo.Format = ''', varargin{3},...
            ''';']);
    end
end
if nargin>=3 % Time units
    ...
    % Data recording
    if strcmp(recorder.Recording,'on')
        T.addbuffer([genvarname(h.Tscollection.Name) '.TimeInfo.Units = ''' varargin{1} ''';']); 
    end
end
if nargin>=4 % Start date
    ...
    if (~isempty(varargin{2}) && isempty(h.Tscollection.TimeInfo.StartDate)) || ...
        ...
        if viewerH.isTimeseriesViewed(h)
            error(message('MATLAB:tsguis:tscollectionNode:settime:plotformerr'));
        end
        ...
        for r = 1:length(tslist)
            ...
            if strcmp(recorder.Recording,'on')
                T.addbuffer([genvarname(h.Tscollection.Name) '.' tslist{r}.Name '.Events = [];']);
            end
        end
    end
    ...
    % Data recording
    if strcmp(recorder.Recording,'on')
        T.addbuffer([genvarname(h.Tscollection.Name) '.TimeInfo.StartDate = ''' varargin{2} ''';']); 
    end
else
    ...
    % Data recording
    if strcmp(recorder.Recording,'on')
        T.addbuffer([genvarname(h.Tscollection.Name) '.TimeInfo.StartDate = '''';']);
    end
end
...
if strcmp(recorder.Recording,'on')
    ...
    T.addbuffer([genvarname(h.Tscollection.Name) '.Time = tsTime;'],h.Tscollection);
end 
