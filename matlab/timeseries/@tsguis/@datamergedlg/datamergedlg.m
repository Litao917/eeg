...
if isempty(dlg) || ~ishandle(dlg)
    ...
    dlg.Figure = figure('Units','Characters','Position',...
      ...
      'None','Numbertitle','off','Menubar','None','Name',getString(message('MATLAB:tsguis:datamergedlg:datamergedlg:ResampleData')),...
      'Visible','off','closeRequestFcn',@(es,ed) set(dlg,'Visible','off'),...
      'IntegerHandle','off','Resize','off','HandleVisibility','Callback','Tag','ResampleData');
    dlg.Handles.LBLselts = uicontrol('Style','Text','Parent',dlg.Figure,'Units','Characters','Position',...
       [2 37.5 60 1.154],'String',getString(message('MATLAB:tsguis:datamergedlg:datamergedlg:SelectedNode')),'HorizontalAlignment',...
       'Left');
...
if strcmp(dlg.ViewNode.Label,getString(message('MATLAB:tsguis:datamergedlg:datamergedlg:Default')))
    strDefault = [getString(message('MATLAB:tsguis:datamergedlg:datamergedlg:Default')) ' '];
