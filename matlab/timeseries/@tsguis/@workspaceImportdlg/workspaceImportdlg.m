function this = workspaceImportdlg(parent)
...
this = tsguis.workspaceImportdlg; 
this.Parent = parent;
...
this.Handles.Browser.open;
this.Handles.Browser.javahandle.setName('workspaceImportdlgImportView');
...
if ~isempty(this.Handles.Browser) 
    if ~isempty(this.Handles.Browser.javahandle)
    set(handle(this.Handles.Browser.javahandle.getSelectionModel,'callbackproperties'),...
       'ValueChangedCallback',[]);
    this.Handles.Browser.javahandle.cleanup; 
    end
    if ~isempty(this.Handles.Browser.Model)
        this.Handles.Model = [];
    end
end 
