...
if ~isempty(h.IOData.SelectedVariableInfo)
    ...
    data=h.checkTimeFormat(h.IOData.SelectedVariableInfo.varname,'row',tmpRowStr{get(h.Handles.COMBtimeIndex,'Value')});
    % check time format
...
%                     errordlg(getString(message('MATLAB:timeseries:tsguis:workspaceImportdlg:EachTimePoint')), ...
        ...
        for i=1:size(blocks,1)
            ...
            if iscell(tmptime)
                try
                    ...
                    errordlg(getString(message('MATLAB:timeseries:tsguis:workspaceImportdlg:InvalidTimesSelectedCorrectThem')),...
