...
if length(ts)==1
        ...
        if h.Timeseries1~=ts{1} && h.Timeseries2~=ts{1}
            if isempty(varargin)
                if h.Timeseries1~=h.Timeseries2
                    ButtonName = questdlg(getString(message('MATLAB:timeseries:tsguis:tscorrnode:ReplaceWhichTimeSeriesObject')), ...
                        getString(message('MATLAB:timeseries:tsguis:tscorrnode:TimeSeriesTools')), ...
                        ['' h.Timeseries1.Name '' ' '],['' h.Timeseries2.Name '' '  '], ...
                        getString(message('MATLAB:timeseries:tsguis:tscorrnode:Cancel')), ...
                        getString(message('MATLAB:timeseries:tsguis:tscorrnode:Cancel')));
                    switch ButtonName,
                        ...
                        case ['' h.Timeseries2.Name '' '  '],
                            h.Timeseries2 = ts{1};
                        case getString(message('MATLAB:timeseries:tsguis:tscorrnode:Cancel'))
                            return
                    end 
                    h.Plot.addTs(h.Timeseries1,h.Timeseries2);
                else
   ...
   if ~isempty(h.Timeseries1) || ~isempty(h.Timeseries2)
       ButtonName = questdlg(getString(message('MATLAB:timeseries:tsguis:tscorrnode:ReplaceTimeSeries')), ...
           getString(message('MATLAB:timeseries:tsguis:tscorrnode:TimeSeriesTools')), ...
           getString(message('MATLAB:timeseries:tsguis:tscorrnode:OK')), ...
           getString(message('MATLAB:timeseries:tsguis:tscorrnode:Cancel')), ...
           getString(message('MATLAB:timeseries:tsguis:tscorrnode:Cancel')));
       if strcmp(ButtonName, ...
           getString(message('MATLAB:timeseries:tsguis:tscorrnode:Cancel')))
           return
       end
   end
...
viewpanel = tsguis.uitspanel('Parent',h.Figure,'Name', ...
    getString(message('MATLAB:timeseries:tsguis:tscorrnode:TimeSeriesCorrelation')),...
