...
h.Handles.LBLtsxlabel = uicontrol('Style','text','String', ...
    getString(message('MATLAB:timeseries:tsguis:tscorrnode:TimeSeries1')),...
    'HorizontalAlignment','left','Parent',h.Handles.PNLXts,'Units','characters');
h.Handles.BTNtsx= uicontrol('Style','pushbutton','String', ...
    getString(message('MATLAB:timeseries:tsguis:tscorrnode:Select')),...
    'Parent',h.Handles.PNLXts,'Units','characters');
h.Handles.TXTtsx = uicontrol('Style','text','HorizontalAlignment','left',...
    'Parent',h.Handles.PNLXts,'Units','characters','Backgroundcolor',[.9 .9 .9]);

%% Xtable
h.Handles.tableModelX = tsMatlabCallbackTableModel(cell(0,2),...
    {getString(message('MATLAB:timeseries:tsguis:tscorrnode:Name')), ...
    getString(message('MATLAB:timeseries:tsguis:tscorrnode:NumberOfColumns'))},[],[]);
h.Handles.tableModelX.setEditable(false);
drawnow
h.Handles.tsTableX = javaObjectEDT('com.mathworks.mwswing.MJTable',...
    h.Handles.tableModelX);
h.Handles.tsTableX.setName('tscorrnode:xtable');
javaMethod('setWidth',h.Handles.tsTableX.getColumnModel.getColumn(0),200);
javaMethod('setWidth',h.Handles.tsTableX.getColumnModel.getColumn(1),100);
javaMethod('setSelectionMode',h.Handles.tsTableX,...
    ListSelectionModel.SINGLE_SELECTION);
javaMethod('setCellSelectionEnabled',h.Handles.tsTableX,false);
javaMethod('setRowSelectionAllowed',h.Handles.tsTableX,true);
javaMethod('setAutoResizeMode',h.Handles.tsTableX,...
    MJTable.AUTO_RESIZE_ALL_COLUMNS);
sPanel = javaObjectEDT('com.mathworks.mwswing.MJScrollPane',h.Handles.tsTableX);
...
h.Handles.LBLtsylabel = uicontrol('Style','text','String', ...
    getString(message('MATLAB:timeseries:tsguis:tscorrnode:TimeSeries2')),...
    'HorizontalAlignment','left','Parent',h.Handles.PNLYts,'Units','characters');
h.Handles.BTNtsy= uicontrol('Style','pushbutton','String', ...
    getString(message('MATLAB:timeseries:tsguis:tscorrnode:Select')),...
    'Parent',h.Handles.PNLYts,'Units','characters');
h.Handles.TXTtsy = uicontrol('Style','text','HorizontalAlignment','left',...
    'Parent',h.Handles.PNLYts,'Units','characters','Backgroundcolor',[.9 .9 .9]);

% Y Table
h.Handles.tableModelY = tsMatlabCallbackTableModel(cell(0,2),...
    {getString(message('MATLAB:timeseries:tsguis:tscorrnode:Name')), ...
    getString(message('MATLAB:timeseries:tsguis:tscorrnode:NumberOfColumns'))},[],[]);
h.Handles.tableModelY.setEditable(false);
drawnow
h.Handles.tsTableY = javaObjectEDT('com.mathworks.mwswing.MJTable',...
    h.Handles.tableModelY);
h.Handles.tsTableY.setName('tscorrnode:ytable');
...
[pathnames,tsnames] = dir(h.getRoot);
...
%% Write the selected row to the label
