...
if ~isempty(h.IOData.SelectedVariableInfo)
    ...
    data=h.checkTimeFormat(h.IOData.SelectedVariableInfo.varname,'column',...
        tmpColStr{get(h.Handles.COMBtimeIndex,'Value')});
    % check time format
...
%                     errordlg('Each time point in the time vector stored in a cell array must have the same size and same format.',getString(message('MATLAB:tsguis:matImportdlg:getTimeColumn:TimeSeriesTools')));
%                     time=[];
%                     timeFormat=[];
%                     return
                end
        ...
        for i=1:size(blocks,1)
            ...
            if iscell(tmptime)
                try
                    ...
                    errordlg(getString(message('MATLAB:tsguis:matImportdlg:getTimeColumn:InvalidTimesSelected')),...
                       getString(message('MATLAB:tsguis:matImportdlg:getTimeColumn:TimeSeriesTools')),'modal');
                    time=[];
                    timeFormat=[];
                    return
                end
