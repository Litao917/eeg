...
%                     errordlg('Each time point in the time vector stored in a cell array must have the same size and same format.',getString(message('MATLAB:tsguis:matImportdlg:getTimeRow:TimeSeriesTools')));
%                     time=[];
%                     timeFormat=[];
%                     return
                end
        ...
        for i=1:size(blocks,1)
            ...
            if iscell(tmptime)
                try
                    ...
                    errordlg(getString(message('MATLAB:tsguis:matImportdlg:getTimeRow:InvalidTimesSelected')),...
                       getString(message('MATLAB:tsguis:matImportdlg:getTimeRow:TimeSeriesTools')),'modal');
                    time=[];
                    timeFormat=[];
                    return
                end
