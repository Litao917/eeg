...
if strcmp(h.State,'DataSelect') 
    ...
    try
        ...
        if all(I)
            errordlg(getString(message('MATLAB:timeseries:tsguis:xyplot:NotRemovingAllPoints')),...
        ...
        % Suspend warnings between updating time series since they will
        ...
        if strcmp(recorder.Recording,'on')
            ts1Name = genvarname(h.Responses.DataSrc.Timeseries.Name);
            ts2Name = genvarname(h.Responses.DataSrc.Timeseries2.Name);
            I = false(size(xdata,1),1);
        ...
        if nargin>=2 && strcmp(varargin{1},'complement')
            ...
            t1 = h.Responses.DataSrc.Timeseries.Time(~I);
            t2 = h.Responses.DataSrc.Timeseries2.Time(~I);
    ...
    catch %#ok<CTCH> % Restore DataChangeEventsEnabled flag if error
        h.Responses.DataSrc.Timeseries.DataChangeEventsEnabled = true;
        h.Responses.DataSrc.Timeseries2.DataChangeEventsEnabled = true;
    end
end