...
if h.Handles.TabPane.isValid
    awtinvoke(h.Handles.TabPane,'add(Ljava/lang/String;Ljava/awt/Component;)',...
        java.lang.String(getString(message('MATLAB:tsguis:corrplot:lagpanel:DefineLags'))),outerLagPanel);
else
    h.Handles.TabPane.add(getString(message('MATLAB:tsguis:corrplot:lagpanel:DefineLags')),outerLagPanel);
end
...
%% Create components
strFrom = getString(message('MATLAB:tsguis:corrplot:lagpanel:From'));
strTo = getString(message('MATLAB:tsguis:corrplot:lagpanel:To'));
LBLnumlagLeft = MJLabel(sprintf('%s :',strFrom));
h.Handles.TXTnumlagLeft = MJTextField(12);
h.Handles.TXTnumlagLeft.setName('TXTnumlagFrom');
awtinvoke(h.Handles.TXTnumlagLeft,'setText(Ljava.lang.String;)','-10');
LBLnumlagRight = MJLabel(sprintf('%s :',strTo));
h.Handles.TXTnumlagRight = MJTextField(12);
h.Handles.TXTnumlagRight.setName('TXTnumlagTo');
...
h.Handles.HelpButton = MJButton(getString(message('MATLAB:tsguis:corrplot:lagpanel:Help')));
