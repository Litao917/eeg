...
for row=1:charTable.getRowCount
    if ~isempty(view.waves(1).Characteristics)
        charMatch = strcmp(get(view.waves(1).Characteristics,'Identifier'),...
                char(charTable.getValueAt(row-1,1)));
        if charMatch
            thischar = view.waves(1).Characteristics;
            if strcmp(thischar.Visible,'on') 
                 newData{row,1} =  true;
            end
