...
%% which may be different from its current length. Used by the "Apply"
...
if nargin>=5 %% Format
    ...
    if strcmp(recorder.Recording,'on')
        T.addbuffer([genvarname(h.Timeseries.Name), '.TimeInfo.Format = ''', varargin{3},...
            ''';']);
    end
end
if nargin>=3 % Time units
    ...
    % Data recording
    if strcmp(recorder.Recording,'on')
        T.addbuffer([genvarname(h.Timeseries.Name) '.TimeInfo.Units = ''' varargin{1} ''';']);
        T.addbuffer('%% Update event units to match the timeseries time units.');
        if ~isempty(h.Timeseries.Events)
            T.addbuffer(['for n = 1:length(',genvarname(h.Timeseries.Name),'.Events)']);
            if ~isempty(h.Timeseries.TimeInfo.StartDate)
                T.addbuffer(['    ','convFact = tsunitconv(''',varargin{1}, ''',', ...
                    genvarname(h.Timeseries.Name) '.Events(n).Units);']);
                T.addbuffer(['    ',genvarname(h.Timeseries.Name) '.Events(n).Time = ',...
                    genvarname(h.Timeseries.Name),'.Events(n).Time*convFact;']);
            end
            T.addbuffer(['    ',genvarname(h.Timeseries.Name) '.Events(n).Units = ''' varargin{1} ''';']);
            T.addbuffer('end');
        end
    end
end
...
if nargin>=4 % Start date
    ...
    if (~isempty(varargin{2}) && isempty(h.Timeseries.TimeInfo.StartDate)) || ...
        ...
        if viewerH.isTimeseriesViewed(h)
            error(message('MATLAB:tsguis:tsnode:settime:plotformerr'));
        end
        ...
        if strcmp(recorder.Recording,'on')
            ...
            T.addbuffer([genvarname(h.Timeseries.Name) '.Events = [];']);
        end
    end
    ...
    % Data recording
    if strcmp(recorder.Recording,'on')
        T.addbuffer([genvarname(h.Timeseries.Name) '.TimeInfo.StartDate = ''' varargin{2} ''';']); 
    end
else
    ...
    % Data recording
    if strcmp(recorder.Recording,'on')
        T.addbuffer([genvarname(h.Timeseries.Name) '.TimeInfo.StartDate = '''';']);
    end
end
...
if strcmp(recorder.Recording,'on')
    ...
    T.addbuffer([genvarname(h.Timeseries.Name) '.Time = tsTime;'],h.Timeseries);
end 
