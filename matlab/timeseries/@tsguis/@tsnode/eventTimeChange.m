...
if col==3
    ...
    if ~isempty(I)
        ...
        if strcmp(recorder.Recording,'on')
            T.addbuffer(['%% ' getString(message('MATLAB:timeseries:tsguis:tsnode:ModifyingEventTime'))]);
            T.addbuffer(['[e,J] = findEvent(' genvarname(h.Timeseries.Name) '.Events,' ...
                '''', h.Timeseries.events(I(1)).Name, ''');']);
            T.addbuffer(['e.Time = ' num2str(evtime) ';'],h.Timeseries);
            T.addbuffer([genvarname(h.Timeseries.Name), '.Events(J) = e;']);
        end
...
elseif col==1
    ...
    % Record action
    if strcmp(recorder.Recording,'on')
        T.addbuffer(['%% ' getString(message('MATLAB:timeseries:tsguis:tsnode:ModifyingEventName'))]);
        T.addbuffer(['[e,J] = findEvent(', genvarname(h.Timeseries.Name) '.Events,',...
...
elseif col==2
    ...
    if strcmp(recorder.Recording,'on') && ~isempty(descr)
        T.addbuffer(['%% ' getString(message('MATLAB:timeseries:tsguis:tsnode:ModifyingEventDescription'))]);
        T.addbuffer(['[e,J] = findEvent(', genvarname(h.Timeseries.Name) '.Events,',...
            '''', genvarname(h.Timeseries.events(startRow).Name), ''');']);
...
eventTable.getModel.setDataVector(tableData, ...
