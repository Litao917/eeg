...
h.Handles.TabPane.add(getString(message('MATLAB:tsguis:histplot:binpnl:DefineBins')),outerBinPanel); 
h.Handles.TabPane.setName('BinPnl.TabPane');
...
%% Create components
strUniformCenters = getString(message('MATLAB:tsguis:histplot:binpnl:UniformCenters'));
strCustomCenters = getString(message('MATLAB:tsguis:histplot:binpnl:CustomCenters'));
strNumberOfBins = getString(message('MATLAB:tsguis:histplot:binpnl:NumberOfBins'));
strVectorOfCenters = getString(message('MATLAB:tsguis:histplot:binpnl:VectorOfCenters'));
h.Handles.RADIOCenters = MJRadioButton(sprintf('%s      ',strUniformCenters));
h.Handles.RADIOCenters.setName('histplot:binpnl:uniform');
h.Handles.RADIOCenters.setSelected(true);
h.Handles.RADIOCustom = MJRadioButton(sprintf('%s      ',strCustomCenters));
h.Handles.RADIOCustom.setName('histplot:binpnl:custom');
BTNGRP = ButtonGroup;
BTNGRP.add(h.Handles.RADIOCenters);
BTNGRP.add(h.Handles.RADIOCustom);
LBLnumbins = MJLabel(sprintf('%s:',strNumberOfBins));
LBLcenters = MJLabel(sprintf('%s:',strVectorOfCenters));
...
h.Handles.HelpButton = MJButton(getString(message('MATLAB:tsguis:histplot:binpnl:Help')));
...
%% When text has changed, select the corresponding radio button
