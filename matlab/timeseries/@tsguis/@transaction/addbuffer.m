        ...
        for k=1:length(tsnodes)
            if isa(tsnodes(k),'tsguis.tscollectionNode')
                ...
                if localTsCompare(tsList,varargin{1})
                    warndlg(getString(message('MATLAB:timeseries:tsguis:transaction:RecordedCodeWillTreatMembers')),...
                        getString(message('MATLAB:timeseries:tsguis:transaction:TimeSeriesTools')),'modal')
                    break
                end
            end
        end
...
%% with another timeseries in the list, if so give it a new name
newName = ts.Name;
oldName = ts.Name;
k = 1;
