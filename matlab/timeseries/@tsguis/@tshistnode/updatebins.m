...
if isempty(h.Dialog) || get(h.Handles.RADIOuniform,'Value')   
    ...
    if ~isempty(numbins) && isscalar(numbins) && numbins>1 && numbins<1e6
        ...
        errordlg(getString(message('MATLAB:timeseries:tsguis:tshistnode:InvalidNumberOfBins')), ...
            getString(message('MATLAB:timeseries:tsguis:tshistnode:TimeSeriesTools')),'modal')
    ...
    if isempty(bins) || ~(all(isfinite(bins)) && ndims(bins)==2 && ...
           ...
           errordlg(getString(message('MATLAB:timeseries:tsguis:tshistnode:InvalidBinVector')), ...
               getString(message('MATLAB:timeseries:tsguis:tshistnode:TimeSeriesTools')),'modal')
