...
%% Overloaded to circumvent the behavior of the @waveform localize method
%% which will not allow a data src to direct all components of a response to
%% the same channel
...
if ~isa(src, 'wrfc.datasource')
  error(message('MATLAB:tsguis:timeplot:addwave:invDatSrc'))
end
wf.DataSrc = src;
wf.Name = src.Name;
