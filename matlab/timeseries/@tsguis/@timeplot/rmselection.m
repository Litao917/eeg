    ...
    if h.Waves(k).isvisible
            ...
            if size(data,2)>1
                ...
                if strcmp(recorder.Recording,'on')              
                    h.SelectionStruct.History = [h.SelectionStruct.History;...
                        {tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'I# = any(I#'')'';')}];
        ...
        elseif strcmp(h.State,'TimeSelect')
            ...
            if strcmp(recorder.Recording,'on')
                ...
                tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'I# = false(',size(h.Waves(k).Data.Time),');')};
            end
            for j=1:size(h.Waves(k).View.SelectedTimes,1)
                ...
                if strcmp(recorder.Recording,'on')
                     h.SelectionStruct.History = [h.SelectionStruct.History; ...
                     {tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),...
        ...
        if ~isempty(I) && ((all(I) && ~complementSelection) || ...
            ...
            errordlg(getString(message('MATLAB:timeseries:tsguis:timeplot:RemovingAllDataWillCreateAnEmptyTimeSeries')),...
        ...
        elseif ~isempty(I) && ((any(I) && ~complementSelection) || ...
            ...
            if strcmp(recorder.Recording,'on')
                T.addbuffer(['%% ' getString(message('MATLAB:timeseries:tsguis:timeplot:SelectingDataForRemoval'))]);
                for row=1:length(h.SelectionStruct.History)
                    T.addbuffer(h.SelectionStruct.History{row});
                end
                if complementSelection
                   T.addbuffer(tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),...
