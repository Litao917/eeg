...
for k=1:length(h.Waves)
    if h.Waves(k).isvisible  
        ...
        elseif strcmp(h.State,'TimeSelect')
            ...
            if strcmp(recorder.Recording,'on')
                h.SelectionStruct.History = ...
                    tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'I# = false(size(#.Time));');
            end
        ...
        if ~isempty(I) && any(any(I))
            ...
            if strcmp(recorder.Recording,'on')
                T.addbuffer(['%% ' getString(message('MATLAB:timeseries:tsguis:timeplot:ReplacingDataWithNaNs'))]);
                for row=1:length(h.SelectionStruct.History)
                    T.addbuffer(h.SelectionStruct.History{row});
                end
                T.addbuffer(sprintf('%s.Data(I%s) = NaN;',genvarname(h.Waves(k).DataSrc.Timeseries.Name),...
                    genvarname(h.Waves(k).DataSrc.Timeseries.Name)),h.Waves(k).DataSrc.Timeseries);
                h.SelectionStruct.History = {};
            end
        end
    end
