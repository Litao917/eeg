...
%% Add the new waveform. Overloaded method used to circumvent the behavior
...
if length(h.Waves)==1 && ~suppressDraw
    if ismac
        h.axesgrid.Title = getString(message('MATLAB:timeseries:tsguis:timeplot:PlotOf',ts.Name));
    else
        h.axesgrid.Title = getString(message('MATLAB:timeseries:tsguis:timeplot:TimeSeriesPlotOf',ts.Name));
    end
else
    h.axesgrid.Title = getString(message('MATLAB:timeseries:tsguis:timeplot:MultipleTimeSeries'));
end
...
if ~suppressDraw
    ...
    if ts.TimeInfo.Length>v.MaxPlotLength
        msg = getString(message('MATLAB:timeseries:tsguis:timeplot:TimeSeriesLengthTooLong', ...
            v.MaxPlotLength,v.MaxPlotLength));
