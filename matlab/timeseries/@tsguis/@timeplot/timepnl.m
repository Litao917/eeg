...
h.Handles.TabPane.add(getString(message('MATLAB:timeseries:tsguis:timeplot:DefineDomain')),TimePnl);
h.Handles.TabPane.setName('TimePnl.TabPane');
...
%% Set the callbacks
TimePnl.TXTStartTime.setName('TXTStartTime');
TimePnl.TXTEndTime.setName('TXTEndTime');
TimePnl.BTNResetTime.setName('BTNResetTime');
...
if h.Handles.TimePnl.RADIOAbsTime.isSelected
    if h.Handles.TimePnl.RADIOStartTime.isSelected
        try
            ...
            errordlg(getString(message('MATLAB:timeseries:tsguis:timeplot:InvalidStartTime')),...
    ...
    if h.Handles.TimePnl.RADIOEndTime.isSelected    
        try
           ...
           errordlg(getString(message('MATLAB:timeseries:tsguis:timeplot:InvalidEndTime')), ...
               getString(message('MATLAB:timeseries:tsguis:timeplot:TimeSeriesTools')),'modal')
           updatetime(view,h);
           return
        end
...
elseif endtime==starttime % 1e-4 is the rounding error produced by sprintf('%0.4g
    ...
    errordlg(getString(message('MATLAB:timeseries:tsguis:timeplot:EndTimePrecedsStartTime')), ...
        getString(message('MATLAB:timeseries:tsguis:timeplot:TimeSeriesTools')),'modal')
...
if h.Handles.TimePnl.RADIORelTime.isSelected
    ...
    if isempty(view.StartDate)
        h.Handles.TimePnl.showErrorDlg(getString(message('MATLAB:timeseries:tsguis:timeplot:TheTimePlotCannotBeSet')));
    else
        view.settimemode(h,'absolute');
    end
end
