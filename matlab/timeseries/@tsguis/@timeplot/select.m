...
if isa(arg1,'tsdata.timeseries')
    ...
    if ~isempty(idx) && ~h.Waves(idx).Data.Exception
        ...
        if h.Waves(idx).data.Reference+length(h.Waves(idx).data.Time)<size(I,1) && ...
            ...
            warndlg(getString(message('MATLAB:timeseries:tsguis:timeplot:CachingOfLargeDatasets')),...
        ...
        if ~h.Waves(k).Data.Exception
            ...
            for j=1:length(h.Waves(k).View.Curves)
                if isequal(ancestor(h.Waves(k).View.Curves(j),'Axes'),double(ax))
                    L = [L;h.Waves(k).View.Curves(j)]; %#ok<AGROW>
                end
            end
            for j=1:length(L)
                xdata = get(L(j),'xdata');
                ...
                if strcmp(recorder.Recording,'on')
                    if newslection || isempty(h.SelectionStruct.History)
                        h.SelectionStruct.History = [h.SelectionStruct.History; ...
                            {tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'I# = false(size(#.Data));')}];
                    end
                    h.SelectionStruct.History = [h.SelectionStruct.History; ...
                            {tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'ind = (#.Time>=',minx,');');...
                            tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'ind = ind & (#.Time<=',maxx,');');...
                            tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'ind = ind  & (#.Data(:,',idx,')>=',miny,');');...
                            tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'ind = ind & (#.Data(:,',idx,')<=',maxy,');');...
                            tsParseBufferStr(genvarname(h.Waves(k).DataSrc.Timeseries.Name),'I#(:,',idx,') = (I#(:,',idx, ') | ind);')}];                                         
                end
            end
        end
    end
