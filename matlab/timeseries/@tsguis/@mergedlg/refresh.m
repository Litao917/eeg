...
%% Specifically, enables and disables radio buttons and displays the
%% "common" time vector
...
if isempty(h.ViewNode.getTimeSeries)
    ...
    if isempty(fullPathSelected) % Disable all components
        ...
        start = getString(message('MATLAB:timeseries:tsguis:mergedlg:Empty'));
        finish = getString(message('MATLAB:timeseries:tsguis:mergedlg:Empty'));
    ...
    elseif length(fullPathSelected)==1 % Single time series resample
        ...
        % default radio and select this time series in the corresponding
        % time series combo
    ...
    set(h.Handles.TXTstarttime,'String',...
        getString(message('MATLAB:timeseries:tsguis:mergedlg:StartTime',start)));
    set(h.Handles.TXTendtime,'String', ...
        getString(message('MATLAB:timeseries:tsguis:mergedlg:EndTime',finish)));
end
