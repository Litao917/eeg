...
for row=1:size(charlist,1)
    if localfindchar(h,charlist{row,:})           
        ...
        if ~isempty(varchar)
            ...
            tableData = [tableData; ...
                        {strcmp(varchar.Visible,'on'), charlist{row,1}} additionalCells];
        else 
            tableData = [tableData; ...
                        {false, charlist{row,1}} additionalDataPropDefaults];
        end
    else
        tableData = [tableData; ...
                     {false, charlist{row,1}} additionalDataPropDefaults]; %#ok<*AGROW>
    end
end
...
if ~isempty(thisTab) && isfield(thisTab.Handles,'CharTable') && ...
    ...
    if nargin<=3
        tableModel = tsMatlabCallbackTableModel(tableData,...
              [{getString(message('MATLAB:timeseries:tsguis:propeditor:Showyn')),...
                getString(message('MATLAB:timeseries:tsguis:propeditor:Name'))} additionalHeadings],...
...
if isempty(thisTab)
    ...
    javaMethod('add',propeditor.Handles.TabPane,...
        getString(message('MATLAB:timeseries:tsguis:propeditor:DefineStatisticalAnnotations')),...
        container);
