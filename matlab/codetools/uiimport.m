...
%   column delimiter.
%
%   See also LOAD, CLIPBOARD, IMPORTDATA.
...
if nargin == 0
    ...
    requestedAction = questdlg(getString(message('MATLAB:codetools:uiimport:SelectADataInputSource')), getString(message('MATLAB:codetools:uiimport:SelectSource')),...
...
if ~useClipboard && ~isSynchronous && isTextFile(fileAbsolutePath)
...
state =  ~isSynchronous && (strcmpi(type, 'xls') || strcmpi(type, 'xlsx') || ...
            strcmpi(type, 'xlsm') || strcmpi(type, 'xlsb') ||  ...
            strcmpi(type, 'ods')|| strcmpi(type, 'xltm') || strcmpi(type, 'xltx'));
end
...
function delimiter = guessdelim(filename)
...
frewind(fid);
delimiter = strguessdelim(str);
fclose(fid);

end

function delim = strguessdelim(str)
