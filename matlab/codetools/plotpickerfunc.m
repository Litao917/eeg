...
if strcmp(action,'defaultshow')
    ...
    switch lower(fname)
        ...
        case {'bar','barh'} 
            ...
            if n==1                
                toshow = (isnumeric(x) || islogical(x)) && ~isscalar(x) && ...
                    ismatrix(x) && (isvector(x) || isreal(x));
            elseif n==2 || n==3
        ...
        case {'barstacked','barhstacked'} 
            ...
            if n==1                
                toshow = (isnumeric(x) || islogical(x)) && ~isscalar(x) && ...
                    ismatrix(x) && ~isvector(x);
            elseif n==2 || n==3
                toshow = localAreaArgFcn(inputvals);
                if isvector(inputvals{2}) % Stacked should not show for single bar plots
                    toshow = false;
                end
                % Check for unique bins if time performance allows 
                if toshow && isvector(x) && length(x)<=1000
        ...
        case {'pie','pie3'} 
            if n==1
                x = inputvals{1};
                toshow = isnumeric(x) && ~isscalar(x) && ismatrix(x) && isreal(x) && ...
                    isfloat(x);
            elseif n==2
                x = inputvals{1};
                y = inputvals{2};
                toshow = isnumeric(x) && ~isscalar(x) && ismatrix(x) && isfloat(x);
        ...
        case 'hist' 
            ...
            elseif n==2
                x = inputvals{1};
                y = inputvals{2};
                toshow = isnumeric(x) && isnumeric(y) && ~isscalar(x) && ismatrix(x) && ...
                    isfloat(x);
        ...
        case {'semilogx','semilogy','loglog'} 
            if n==1
                x = inputvals{1};
                toshow = (isnumeric(x) || islogical(x)) && ~isscalar(x) && ismatrix(x);
            elseif n==2
                toshow = localAreaArgFcn(inputvals);
            elseif n==3
        ...
        case 'plotmatrix' 
            if n==1
                x = inputvals{1};
                toshow = (isnumeric(x) || islogical(x)) && ismatrix(x) && ...
            ...
            elseif n==2
                ...
                toshow = (isnumeric(x) || islogical(x)) && (ischar(y) || ...
                    ...
                    size(x,2)>1 && size(y,2)>1 && ismatrix(x) && ismatrix(y))) && ...
                    isreal(x) && isreal(y);
            elseif n==3
                x = inputvals{1};
                y = inputvals{2};
        ...
        case 'spy' 
            if n==1
                s = inputvals{1};
                toshow = (isnumeric(s) || islogical(s)) && ~isscalar(s) && ismatrix(s);
            elseif n==2
                s = inputvals{1};
                l = inputvals{2};
                toshow = (isnumeric(s) || islogical(s)) && ~isscalar(s) && ismatrix(s);
        ...
        case 'compass' %1 or 2 vectors or matrixes of compatible size with optional linepsec string
            if n==1
                ...
                toshow = (isnumeric(u) || islogical(u)) && ~isscalar(u) && ...
                    ismatrix(u) && isfloat(u);
            elseif n==2
                u = inputvals{1};
                v = inputvals{2};
                toshow = isnumeric(u) && ~isscalar(u) && ismatrix(u) && isfloat(u) && ...
                    isfloat(v);
        ...
        case 'pcolor' %1 color array or 2 vectors and an color array of compatible size
            ...
            elseif n==3
                ...
                if isvector(x) && isnumeric(x) && isvector(y) && ...
                    ...
                    if ismatrix(C) && size(C,1)==length(y) && size(C,2)==length(x)
                        toshow = true;
                    elseif ndims(C)==3 && size(C,3)==3
                        toshow = max(C(:))<=1 && min(C(:))>=0;
                    else
        ...
        case 'ribbon' %1 matrix with 2 matrices or vectors of the same size with an optional scalar width parameter
            if n==1
                x = inputvals{1};
                toshow = isnumeric(x) && ismatrix(x) && size(x,1)>1 && size(x,2)>1;
            elseif n==2
                x = inputvals{1};
                y = inputvals{2};
        ...
        case {'ezcontour','ezcontourf'} % A 2-input function handle with an optional integer grid parameter or a 2 or 4 element domain vector
             ...
             elseif n==2
                ...
                toshow = isa(fcn,'function_handle') && nargin(fcn)==2 && ...
                    ...
                    isequal(size(domain),[4 1]) || (ismatrix(domain) && ...
        ...
        case 'streamline' 
            ...
            if n==1 && ~isempty(inputvals{1}) && iscell(inputvals{1}) && isvector(inputvals{1}) && ...
...
elseif strcmp(action,'defaultdisplay') 
    ...
    dispStr = '';
    switch lower(fname)
        case {'plot','semilogx','semilogy','loglog','area','stem',...
                'stairs','bar','barh'}
            if (n==1 && (isvector(inputvals{1}) || isa(inputvals{1},'timeseries') || ...
                    isa(inputvals{1},'fints')  )) || (n>=2 && isvector(inputvals{2})) 
        ...
        case 'scatter'
           ...
           elseif n==1
               if length(regexpi(inputnames{1},'\(.*\)'))==1 
                  dispStr =  [lower(fname) '(getcolumn(' inputnames{1} ',1),getcolumn(' inputnames{1} ',2))'];
               else
                  dispStr =  [lower(fname) '(' inputnames{1} '(:,1),' inputnames{1} ')'];
               end
           elseif n==3
        ...
        case 'plot as multiple series' 
            dispStr = [dispStr sprintf('plot(%s,''DisplayName'',''%s'');hold all;',inputnames{1},inputnames{1})];
    ...
    switch lower(fname)
        ...
        case 'scatter'
            if n==1 
                vardata = inputvals{1};
                varname = inputnames{1};
                if ismatrix(vardata) && size(vardata,2)==2
                    if length(regexpi(varname,'\(.*\)'))==1  
