...
if strcmpi(option,'dir')
    if isdir(name) %check for valid folder
        ...
        fileList = internal.matlab.codetools.reports.matlabFiles(dirname, reportName);
    else
...
elseif strcmpi(option,'file')
    ...
    if exist(name,'file') %make sure the file is valid
        ...
        internal.matlab.codetools.reports.webError(getString(message('MATLAB:codetools:reports:FileNotFoundSoft',name)), reportName);
        return
    end
else % user has to specify either 'file' or 'dir'
    internal.matlab.codetools.reports.webError(getString(message('MATLAB:codetools:reports:InvalidDirFileOption', option)), reportName);
    return
end
...
if ~isempty(strc)
    ...
    for n = 1:length(strc)
        ...
        reportComponent = sprintf('%s', strrep(strc(n).filename,'.m',''));
