...
if strcmp(option,'dir')
    dirname = name;
    fileList = internal.matlab.codetools.reports.matlabFiles(name, reportName); 
    %Tag on the directory path.
    fileList = strcat(name,filesep,fileList);
else
    fullname = which(name);
    if isempty(fullname)        
        internal.matlab.codetools.reports.webError(getString(message('MATLAB:codetools:reports:SpecificFileNotFound',name)), reportName);
        return
    end
    fileList = {fullname};   
end
...
%the table header
s{end+1} = ['<tr><td valign="bottom" width="20%%"><strong>', getString(message('MATLAB:codetools:reports:MATLABFileList')) , '</strong></td>'];
if allChildDisplayMode
    s{end+1} = ['<td valign="bottom">', getString(message('MATLAB:codetools:reports:ChildrenCalledFunctions')), '</td>'];
end
if localParentDisplayMode
    s{end+1} = ['<td valign="bottom">', getString(message('MATLAB:codetools:reports:ParentsCallingFunctions')), '</td>'];
end
s{end+1} = '</tr>';
...
for fileIndex = 1:length(childStrc) %for each file
    ...
    for subFunIndex = 1:N
        ...
        s{end+1} = sprintf('<a href="matlab: [~] = matlab.desktop.editor.openAndGoToLine(''%s'',%d);"><span class="mono">%s</span></a></td>', ...
...
for i = 1: length(currChildStrc)
    ...
    switch(callType)
        case ReportConstants.Unknown
            ...
            s{end+1} = sprintf('<a href="matlab: [~] = matlab.desktop.editor.openAndGoToLine(''%s'',%d);">%s</a></span><br/>', ...
                filename, ...
                thisCallStrc.line, ...
                thisCallStrc.name);
        ...
        otherwise %a toolbox or other that might have an overloaded method
            ...
            if hasAlternatives
                ...
                s{end+1} = sprintf([getString(message('MATLAB:codetools:reports:MultipleClassMethodsFound')), ' ', '<a href="matlab: [~] = matlab.desktop.editor.openAndGoToLine(''%s'',%d);">%s.m</a>'], ...
                    filename, thisCallStrc.line, justname);
            else
                s{end+1} = sprintf('<a href="matlab: [~] = matlab.desktop.editor.openAndGoToLine(''%s'',%d);">%s</a>', ...
                    filename, ...
                    thisCallStrc.line, ...
                    thisCallStrc.name);
            end
