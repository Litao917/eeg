...
if strcmp(option,'dir')
    ...
    if isempty(fullname)
        internal.matlab.codetools.reports.webError(getString(message('MATLAB:codetools:reports:FileNotFoundPeriod',name)), title);
        return
    end
    fileList = {name};
end
...
if strcmp(option,'file')
    s{end+1} = sprintf([getString(message('MATLAB:codetools:reports:ReportForFile')) ' <a href="matlab: edit(urldecode(''%s''))">%s</a>'], ...
...
%             urlencode(strc(n).fullfilename), regexprep(strc(n).filename,'\.m$','')); %#ok<AGROW>
        ...
        regExpRep = sprintf('%s', regexprep(strc(n).filename,'\.m$',''));
