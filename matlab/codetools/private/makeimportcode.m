...
switch (outputBreakup)
    case 2
        ...
        hFunc.addText('    ', hNewOutput, '.(genvarname(', hOutputArg, '.rowheaders{i})) = ', hOutputArg, '.data(i, :); %#ok<DEPGENAM>');
        hFunc.addText('end');
    ...
    case 1
        ...
        hFunc.addText('    ', hNewOutput, '.(genvarname(', hOutputArg, '.colheaders{i})) = ', hOutputArg, '.data(:, i); %#ok<DEPGENAM>');
        hFunc.addText('end');
...
switch (outputBreakup) 
    case 1        
        ...
        hFunc.addText('for i = 1:size(', hOutputArg, '.colheaders, 2)');
        hFunc.addText('    assignin(''base'', genvarname(', hOutputArg, '.colheaders{i}), ', hOutputArg, '.data(:,i)); %#ok<DEPGENAM>');
        hFunc.addText('end');
    ...
    case 2
        ...
        hFunc.addText(sprintf('%% %s',strCreateNewVariablesInBaseWorkspace));
        hFunc.addText('for i = 1:size(', hOutputArg, '.rowheaders, 1)');
        hFunc.addText('    assignin(''base'', genvarname(', hOutputArg, '.rowheaders{i}), ', hOutputArg, '.data(i,:)); %#ok<DEPGENAM>');
        hFunc.addText('end');        
...
hFunc.addText(hNewOutputArg, '.(genvarname(name)) = ', hOutputArg, '; %#ok<DEPGENAM>');
...
switch (params.outputBreakup) 
    case 1
        ...
        if ~params.LOADXL && params.hasOutputArg ~= 0
            ...
            hFunc.addText('');
            hFunc.addText(sprintf('%% %s',strBreakDataUpIntoOneFieldPerColumn));
            hFunc.addText('colheaders = genvarname(', hOutputArg ,'.colheaders); %#ok<DEPGENAM>');
    ...
    case 2
        ...
        if ~params.LOADXL && params.hasOutputArg ~= 0
            ...
            hFunc.addText('');
            hFunc.addText(sprintf('%% %s',strBreakDataUpIntoOneFieldPerRow));
            hFunc.addText('rowheaders = genvarname(', hOutputArg ,'.rowheaders); %#ok<DEPGENAM>');
