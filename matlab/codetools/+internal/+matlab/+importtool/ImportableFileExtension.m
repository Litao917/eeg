...
%   notice in a future version. 
    ...
    methods(Static)        
        ...
        function fileExt = getVideoFileExtensions(extentionsWithDotPrefix)
            if nargin < 1
                extentionsWithDotPrefix = true;
            end
            % Return a cell array of supported video file extensions
            fileExt = localVideoExtn(extentionsWithDotPrefix);
        end

        
        % extensionsWithDotPrefix takes true/false. 
        % Ex: true : returns {'.wav'}  false: returns  {'wav'}
        function fileExt = getAudioFileExtensions(extentionsWithDotPrefix)
            if nargin < 1
                extentionsWithDotPrefix = true;
            end
            % Return a cell array of supported audio file extensions
            fileExt = localAudioExtn(extentionsWithDotPrefix);
        end
...
function fileExt = localVideoExtn(extentionsWithDotPrefix)
...
function fileExt = localAudioExtn(extentionsWithDotPrefix)
