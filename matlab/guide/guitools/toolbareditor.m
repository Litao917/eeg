...
elseif feature('HGUsingMATLABClasses')
    hPropertyTabPanel = uitabgroup(...
        ...
        'Units','pixels',...
        'position',[280,70,mFigurePosition(3)-280-20,360],...
        'SelectionChangeCallback',@changePropPanel);
    ...
    hPropertyTabPanel = uitabgroup(...
        ...
        'position',[280,70,mFigurePosition(3)-280-20,360],...
        'SelectionChangeCallback',@changePropPanel);
    safeSetObsoletedTabGroupProperties(hPropertyTabPanel);
    hToolbuttonPropertyTab = uitab(...
        'Parent',hPropertyTabPanel,...
        'Title', getUserString('ToolPropertyPanelTitle'),...
        'Tag','_ToolbuttonPropertyTab');   % UITAB to contain the tool properties/menu
...
function prepareLayout(topContainer)
    ...
    if ~feature('HGUsingMATLABClasses') && ~isempty(find(isprop(allObjects,'TitleHandle'), 1))
        warning('off','MATLAB:Uipanel:HiddenImplementation');
        titles=get(allObjects(isprop(allObjects,'TitleHandle')), 'TitleHandle');
        allObjects(ismember(allObjects,[titles{:}],'legacy')) = [];
...
%------------------------------------------------------------------
function safeSetObsoletedTabGroupProperties(tabgroup)
    assert(ishghandle(tabgroup),'Argument needs to be a hg handle');
    assert(strcmpi(get(tabgroup,'type'),'uitabgroup'),'Argument needs to be a uitabgroup handle');
    try
        set(tabgroup,...
            'BackgroundColor',get(0,'defaultuicontrolbackgroundcolor'),...
            'Margin',1);
