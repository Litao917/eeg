...
% workaround for HGUsingMATLABClasses (close all force deletes GUIDE figure)
if isempty(copyBuffer) || (~matlab.graphics.internal.isGraphicsVersion1 && ~ishghandle(copyBuffer))
...
if ~isExternalControl(obj)
    ...
    % workaround for HGUsingMATLABClasses, long[] not working from Java
    % side through jmi
...
if (flag>0)
    ...
    choiceTitle = getString(message('MATLAB:guide:guidefunc:PromptToContinueActivation'));
    dialog = PathUpdateDialog(frame, getDialogTitle(), filename, choiceTitle, mdbFlag);
    ...
    if ~isempty(data)
        ...
        for i=1:length(hndls)
            ...
            elseif guidemfile('isManagedButtonGroupCallback',hndls{i}, whichCb, source)
                ...
                if isequal(questdlg(...
                        sprintf('%s',getString(message('MATLAB:guide:guidefunc:ModifiedAutoCallback', whichCb, whichCb))), ...
                        getDialogTitle(), ...
...
if ishghandle(obj, 'uicontrol')
    str = get(obj, 'string');
elseif guidemfile('isbuttongroup', obj)
    str = get(obj, 'title');
elseif ishghandle(obj,'uipanel')
    str = get(obj, 'title');
end
...
for i = 1:limit
    ...
    % workaround for HGUsingMATLABClasses
    accountForHGUsingMATLABClasses(obj);
...
if nargin == 2 && ~isempty(filename)
    ...
    % Handle unsupported child objects of the fig file
    handleUnsupportedObjectTypes(fig);
...
% need to change to handle when saving in HGUsingMATLABClasses
...
if isExternalControl(h)
    ...
    for i=1:length(fields)
        try
            % workaround for HGUsingMATLABClasses: uitable peer is 
...
% ****************************************************************************
% Handles update M-file and callbacks when Tag property changes
% ****************************************************************************
...
for i=1:length(eventlist)
    if ~isprop(peer, eventlist{i})
        ...
        createDynamicProperty(peer, eventlist{i}, 'Callback');
...
if ~isprop(peer, 'Peer')
    ...
    createDynamicProperty(peer, 'Peer', 'handle');
end
...
createDynamicProperty(handle(fig), 'GUIDEFigure', 'double');

function clearGuideFigure(fig, varargin)
p = findprop(handle(fig), 'GUIDEFigure');
delete(p);

function createDynamicProperty(obj, propname, proptype)
obj =handle(obj);
...
if ~matlab.graphics.internal.isGraphicsVersion1
    ...
    if isempty(p)
        p = schema.prop(obj, propname, proptype);  
        p.AccessFlags.Serialize = 'off';        
    end
end

% *************************************************************************
% handle HGUsingMATLABClasses incompatibilities that impact GUIDE GUI
% *************************************************************************
function accountForHGUsingMATLABClasses(obj)

% first, the callback of buttons in buttongroup needs to be removed since
% HGUsingMATLABClasses does not use it anymore
if ~matlab.graphics.internal.isGraphicsVersion1
    parent = get(obj,'Parent');
    if ishghandle(obj,'uicontrol') && guidemfile('isbuttongroup',parent) 
...
for i=numChildren:-1:1
    ...
    % NOTE: need to check the type differently for HG1 and HG2
     if ( strcmp(childObjectType,'legend') ||...         % HG2
          strcmp(childObjectType,'colorbar') ||...       % HG2
          isa(childObjectHandle,'scribe.colorbar') ||... % HG1
          isa(childObjectHandle,'scribe.legend') )       % HG1
...
if foundInvalidObject 
    ...
    warningString = getString(message('MATLAB:guide:guidefunc:UnsupportedObjectTypes'));
