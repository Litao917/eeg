function tests = functiontests(fcns)
...
%       and teardown functions are used to setup % and teardown the fresh
...
validateattributes(fcns, {'cell'}, {'nonempty'}, '', 'fcns');

cellfun(@validateScopedFunction, fcns);

% Get the function names
fcnNames = cellfun(@func2str, fcns, 'UniformOutput', false);

% Filter out any non-test functions
areTestFcns = cellfun(...
    @(fcnName) DefaultTestNameMatcher.isTest(fcnName), fcnNames);
testFcns = fcns(areTestFcns);
% Row vectorize
testFcns = testFcns(:)'; 
...
if any(setupFcnIdx(:))
    paramValues = [paramValues {'SetupFcn'}, fcns(setupFcnIdx)];
end

teardownFcnIdx = strcmp(fcnNames, 'teardown');
if any(teardownFcnIdx(:))
    paramValues = [paramValues {'TeardownFcn'}, fcns(teardownFcnIdx)];
end

setupOnceFcnIdx = strcmp(fcnNames, 'setupOnce');
if any(setupOnceFcnIdx(:))
    paramValues = [paramValues {'SetupOnceFcn'}, fcns(setupOnceFcnIdx)];
end

teardownOnceFcnIdx = strcmp(fcnNames, 'teardownOnce');
if any(teardownOnceFcnIdx(:))
    paramValues = [paramValues {'TeardownOnceFcn'}, fcns(teardownOnceFcnIdx)];
end
