classdef (Sealed) HasParameter < matlab.unittest.internal.selectors.Selector
    ...
    methods
        function selector = HasParameter(varargin)
            % HasParameter - Class constructor
            %
            %   selector = HasName(ATTRIBUTE,VALUE) creates a selector that filters
            ...
            %   and 'Name' can be specified as a string and 'Value' can be specified as
            %   any MATLAB datatype.
            ...
            selector.PropertyConstraint = convertStringToConstraint(parser.Results.Property);
            selector.NameConstraint = convertStringToConstraint(parser.Results.Name);
    ...
    methods (Hidden)
        function bool = uses(~, attributeClass)
            bool = attributeClass <= ?matlab.unittest.internal.selectors.ParameterAttribute;
        end
        
        function result = select(selector, attributes)
            for currentAttribute = attributes
                if ~currentAttribute.acceptsParameter(selector);
                    result = false;
                    return;
                end
            end
