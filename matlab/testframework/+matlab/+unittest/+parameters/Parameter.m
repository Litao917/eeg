classdef Parameter < matlab.mixin.Heterogeneous
    ...
    methods (Access=protected)
        function param = Parameter(prop, name)
            ...
            if iscell(default)
                if iscellstr(default)
                    names = makeUniqueStrings(makeValidName(default, 'Prefix','p_'), {}, namelengthmax);
                else
                    names = arrayfun(@(n)sprintf('value%d',n), 1:numel(default), 'UniformOutput',false);
                end
            ...
            if nargin > 1
                ...
                default = struct(name, default.(name));
            end
    ...
    methods (Hidden, Sealed)
        function params = filterByClass(params, desiredClass)
            ...
            actualClasses = arrayfun(@class, params, 'UniformOutput',false);
    ...
    methods (Hidden, Sealed, Static, Access=protected)
        ...
        function parameterNames = getParameterNamesFor(method)
            ...
            % Inputs ignored with '~' are not allowed.
            if any(strcmp(parameterNames, '~')) || any(strcmp(parameterNames, 'varargin'))
                error(message('MATLAB:unittest:Parameter:InvalidMethodInputArgument', ...
                    method.Name, method.DefiningClass.Name));
            end
        ...
        function [status, combined] = combineParameters(parameters, combinationAttribute)
            ...
            switch combinationAttribute
                ...
                case 'sequential'
                    [status, combined] = combineSequentially(parameters);
                case 'pairwise'
                    status = true;
...
function [status, sequentialOrdering] = combineSequentially(parameters)
...
allParameterSizes = cellfun(@numel, parameters);
parameterSize = unique(allParameterSizes);
if numel(parameterSize) ~= 1
...
% LocalWords:  flds vals sz Kuo Tai Yu codetools
