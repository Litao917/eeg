...
% This MATLAB code file should be the only place using it.
...
if nargin == 1 % GET
  ...
  if isempty(data_in) 
      if isappdata(fig,PROP_NAME)
          % this gives us a way for removing the appdata so that it will not be
          % saved in the .fig file
          rmappdata(fig,PROP_NAME);
      end
  else
