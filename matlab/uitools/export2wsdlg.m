...
function [cb, e] = layoutDialog(hDlg, okBut, cancelBut, helpBut, checkboxLabels, ...
    ...
    if graphicsversion(hDlg,'handlegraphics')
        outerPos = localClipOuterPositionToScreen(hDlg);
        set(hDlg, 'OuterPosition', outerPos); 
    else
        set(hDlg,'ResizeFcn',@localResize)
    end
...
%----------------------------------------------------------------------------
function localResize(hDlg,~)

    % Resize function maintains the dialog within the screen bounds
    outerPos = get(hDlg, 'OuterPosition');
    outPositionOffsets = outerPos-get(hDlg, 'Position');    
    outerPos = localClipOuterPositionToScreen(hDlg,outerPos);
    set(hDlg, 'Position', outerPos-outPositionOffsets);
 
%----------------------------------------------------------------------------    
function outerPos = localClipOuterPositionToScreen(hDlg,outerPos)

    % Shift the OuterPosition property so that the entire rectangle is 
