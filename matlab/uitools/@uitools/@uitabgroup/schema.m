...
if (~isa(handle(hChild), 'uitools.uitab'))
    ...
    error(message('MATLAB:schema:CannotBeParent', get( hChild, 'type' ), get( hChild, 'type' )));  
end
...
if (evt > length(children) || evt < 0 || (evt==0 && ~isempty(children)))   
    ...
    error(message('MATLAB:schema:InvalidIndex'));
end
...
if ~(isequal(get(newValue,'Parent'),double(src)))
    error(message('MATLAB:schema:InvalidTab'));
end
