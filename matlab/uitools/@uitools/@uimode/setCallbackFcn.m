...
if strcmp(hThis.Enable,'on')
    ...
    if (ischar(enableState) && strcmpi(enableState,'on')) || ...
        ...
        mmgrEnableFunction = @(hL)graphics.internal.setListenerState(hL,'on');
    else
        mmgrEnableFunction = @(hL)graphics.internal.setListenerState(hL,'off');
    end
    graphics.internal.setListenerState(mmgr.WindowListenerHandles,'off');
    ...
    if (ischar(windowListEnableState) && strcmpi(windowListEnableState,'on')) || ...
        ...
        windowListEnableFunction = @(hL)graphics.internal.setListenerState(hL,'on');
    else
        windowListEnableFunction = @(hL)graphics.internal.setListenerState(hL,'off');
    end
