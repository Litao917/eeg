...
%   Copyright 1984-2006, 2008 The MathWorks, Inc.
...
switch nargin,
  ...
  case 3,
    ...
    if strcmpi(HorizontalAlignment{:},'fixed'),
      HorizontalAlignment={'Fixed',0};
    end      
    if strcmpi(VerticalAlignment{:},'fixed'),
  ...
  %%%%%%%%%%%%%
  otherwise,
    error(message('MATLAB:align:WrongNumberInputs'));
...
if ~isempty(HandleList) & all(ishghandle(HandleList)), % Handles passed in, can be a 2D array, so don't use &&
...
NewHandles=HandleList(ishghandle(HandleList));
...
for ObjLp=2:NumObjs,
  TempPos(ObjLp,HV)=sum(TempPos(ObjLp-1,[HV HV+2]),2)+Gap;
end
...
MaxVal=max(sum(Position(:,[HV HV+2]),2));
