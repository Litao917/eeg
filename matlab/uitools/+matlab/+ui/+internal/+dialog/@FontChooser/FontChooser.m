...
%   Copyright 2007-2013 The MathWorks, Inc.
    ...
    methods
        ...
        %    FontChooser('Title','Atitle','FontName,'Arial')
        % 2. Direct assignment
        %    a = FontChooser
        %    a.FontName = 'Arial'
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function obj = FontChooser(varargin)
            initialize(obj);
            if rem(length(varargin), 2) ~= 0
                error(message('MATLAB:UiFontChooser:UnpairedParamsValues'));
            end

            for i = 1:2:length(varargin)

                if ~ischar(varargin{i})
                    error (message('MATLAB:UiFontChooser:IllegalParameterType', i));
                end
                ...
                if isValidFieldName(obj,fieldname)
                    ...
                    error(message('MATLAB:UiFontChooser:IllegalParameter', varargin{ i }));
                end
            end
            createPeer(obj);
        end
