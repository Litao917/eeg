...
%
%   ARPACKC will be removed in a future release.  Use EIGS instead.
%
%   NOTE:  int refers to int32 if you using a 32-bit machine, and int64
%          if you are using a 64-bit machine.
%
%   [IDO,INFO] = ARPACKC('dsaupd',IDO,BMAT,N,WHICH,NEV,TOL,RESID,NCV,V,LDV,...
...
%   INFO {int}
%
%   [D,INFO] = ARPACKC('dseupd',RVEC,HOWMNY,SELECT,Z,LDZ,SIGMA,...
%                      BMAT,N,WHICH,NEV,TOL,RESID,NCV,V,LDV,...
%                      IPARAM,IPNTR,WORKD,WORKL,LWORKL,INFO)
%   D: eigenvalues. {length NEV double}
%   RVEC: Compute eigenvectors or not. {int}
%   HOWMNY: 'A' for all Ritz vectors, 'S' selects from SELECT. {char}
%   SELECT: which Ritz vectors are computed. {length NCV int}
%   Z: Ritz vectors. {length N*NEV double}
%   LDZ: leading dimension of Z. {int}
%   SIGMA: Scalar eigenvalue shift. {double}
%   Remaining inputs must be unchanged from last 'dsaupd' call.
%
%   [IDO,INFO] = ARPACKC('dnaupd',IDO,BMAT,N,WHICH,NEV,TOL,RESID,NCV,V,LDV,...
%                        IPARAM,IPNTR,WORKD,WORKL,LWORKL,INFO)
%   Same for 'dsaupd', except:
%   WHICH: 'LM','SM','LR','SR','LI','SI'. {length 2 char}
%   LWORKL: length of WORKL >= 3*NCV*(NCV+2) {int}
%
%   [D,INFO] = ARPACKC('dneupd',RVEC,HOWMNY,SELECT,Z,LDZ,SIGMAR,SIGMAI,...
%                      WORKEV,BMAT,N,WHICH,NEV,TOL,RESID,NCV,V,LDV,...
%                      IPARAM,IPNTR,WORKD,WORKL,LWORKL,INFO)
%   Same as 'dseupd' and 'dnaupd' except:
%   D: Ritz values {length NEV+1 double MATLAB (i.e. split) complex}
...
%
%   [IDO,INFO] = ARPACKC('znaupd',IDO,BMAT,N,WHICH,NEV,TOL,RESID,NCV,V,LDV,...
...
%   LWORKL: length of complex WORKL >= 3*NCV^2+5*NCV {int}
%   RWORK: {2*NCV double}
%
%   [D,INFO] = ARPACKC('zneupd',RVEC,HOWMNY,SELECT,Z,LDZ,SIGMA,WORKEV,...
%                      BMAT,N,WHICH,NEV,TOL,RESID,NCV,V,LDV,...
%                      IPARAM,IPNTR,WORKD,WORKL,LWORKL,RWORK,INFO)
...
%
%    MathWorks version of ARPACK includes the patch dated June 2, 2000.
%
%   MathWorks modified ARPACK slightly by increasing the length of IPNTR to
...
%   MathWorks has compiled the top level FORTRAN subroutines:
...
%   The C mex-file ARPACKC processes the MATLAB inputs and calls the
%   corresponding ARPACK routine.  ARPACKC is used by EIGS, which performs
%   the ARPACK reverse communication and matrix application.
%
%   See also EIGS.
