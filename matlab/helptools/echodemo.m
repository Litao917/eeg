...
if ~isempty(thisCell.title)
    fprintf(' %s\n',upper(xlate_wrapper(thisCell.title))); 
end
...
linkPattern = '\<(<\S.*?\S>)\>';
fromLastLine = '';
thisCell.text = xlateParagraphs(thisCell.text);
for n = 1:length(thisCell.text)
    ...
    if ~isempty(image)
        ...
        for iTokens = length(linkTokens):-1:1;
            ...
            noColons = isempty(strfind(linkUrl,':'));
            if noColons
                if linkUrl(1) == '#'
...
function xlatedText = xlateParagraphs(originalText)
% Translate the text if xlate files are available

% Do nothing if the cell contains no text
if isempty(originalText)
    xlatedText = originalText;
    return
end

% Call xlate a paragraph at a time
numLines = numel(originalText);
xlatedText = {};

% Paragraphs are delimited by empty lines.
paragraphBreaks = [0,find(cellfun(@isempty,originalText)),numLines];
numParagraphs = numel(paragraphBreaks)-1;

for paragraphCount = 1:numParagraphs
    paragraphText = originalText(paragraphBreaks(paragraphCount)+1:paragraphBreaks(paragraphCount+1));
    
    % first we convert the text to a single line.
    theText = strcat(paragraphText,'\n');
    theText{end} = paragraphText{end}; % strip the new line from the very end.
    fullText = [theText{:}];
    
    % Now xlate the string.
    xlateFullText = xlate_wrapper(fullText);
    % if xlate didn't change anything then return the original text
    if(isequal(xlateFullText,fullText))
        xlatedParagraph = paragraphText;
    else
        % convert the single string back to a cell array -- note we must
        % transpose this to ensure a row vector.
        xlatedParagraph = strread(xlateFullText,'%s','delimiter','\n')';
    end
    
    % concatenate the paragraphs
    xlatedText = [xlatedText,xlatedParagraph]; %#ok<AGROW>
end
...
function s = xlate_wrapper(s)
% Try to translate with XLATE_DEMOS. If it is not available, fall back to
% XLATE.
