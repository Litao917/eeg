...
if (feature('HGUsingMATLABClasses') == 1)
    ...
    createPlotManagerListener (plotmgr, fig, 'PlotFunctionDone', {@figPastePlotDoneCallback}, 'PlotFunctionDoneLsnr',0);
    createPlotManagerListener (plotmgr, fig, 'PlotEditPaste', {@figPastePlotDoneCallback}, 'PlotEditPasteLsnr',0);
    ...
    for i = length(children):-1:1  % walk backwards through children for correct order
        ...
        elseif isa (handle(child), 'hg.axes')
            ...
            %AII fix for g651031
            createListener (axes, 'ObjectBeingDestroyed', {@objDestroyedCallback}, 'ObjDestroyedLsnr',0);
...
%-------------------------------
    function figPastePlotDoneCallback (~, eventData) 
    ...
    % The refeeshAxes function uses get_legendable_children to decide 
    % which axes children to display, which only includes HandleVisibility 
    % 'on' because it accesses children via a get(axes,'Children').
    % Consequently the axesChildAddedCallback should only include these
    % children. This prevents to Plot Browser reacting to the addition of 
    % handle invisible zoom lines (896807)
    if ~strcmp('on', get(axesChild,'HandleVisibility'))
        return
    end
    ...
    %-------------------------------
    function createPlotManagerListener (pm, fig, ev, callback, uniqueName, recursionLimit)
    % Creates the listener and stores it in the object for safekeeping.

    lsnr = handle.listener (pm, ev, callback);
    ...
    % This guarantees that the event handlers will not pass out of scope
    % and be garbage-collected:
    if ~isprop (handle(fig), 'PlotBrowserListeners')            
        p = schema.prop (handle(fig), 'PlotBrowserListeners', 'MATLAB array');
        p.AccessFlags.Serialize = 'off';
        set (p, 'Visible', 'off');
        set (handle(fig), 'PlotBrowserListeners', struct);
    end
    
    str = get(fig, 'PlotBrowserListeners');
    str.(uniqueName) = lsnr;
    set(fig, 'PlotBrowserListeners', str);  
    ...
    function refreshAxes(ax)
        
        allChildren = graph2dhelper ('get_legendable_children', ax);
        ...
        for j = 1:length(allChildren)
            ...
            % If this axesChild is part of a plotyy axes, it may not be a child of
            % ax. This is a consequence of get_legendable_children returning the
            % children of both the main axes and the peer axes for plotyy (which is
            % necessary for legend to work correctly for plotyy). Distorting the
            % true axes-child is not appropriate for the Plot Browser, which should
            % reflect the real hg hierarchy (g840295)
            if isequal(get(axesChild,'Parent'),ax)
