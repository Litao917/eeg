...
if ~isValid
  if isempty(MissingRequiredField)
    errmsg = getString(message('MATLAB:validOptimProblemStruct:InvalidValueField', ...
                         InvalidValueField));
  elseif isempty(InvalidValueField)
    errmsg = getString(message('MATLAB:validOptimProblemStruct:MissingRequiredField', ...
                         MissingRequiredField));
  else
    errmsg = getString(message('MATLAB:validOptimProblemStruct:InvalidStructInput', ...
                         MissingRequiredField, ...
                         InvalidValueField));
  end
end
...
if isfield(myStruct,'randstate') && isfield(myStruct,'randnstate')
    warning(message('MATLAB:validOptimProblemStruct:DeprecatedRNGFields'));
    ...
    elseif isa(myStruct.randstate, 'uint32') && isequal(size(myStruct.randstate),[625, 1]) && ...
        ...
        try
            ...
            error(message('MATLAB:validOptimProblemStruct:InvalidRNGFields'));
        end
        warning(warnState);
