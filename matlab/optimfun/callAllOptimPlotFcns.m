...
if(strcmp(flag,'init')) || isempty(findobj(0,'Type','figure','name',fname))
    ...
    if buttonsOK
        ...
        stopBtnXYLoc = [5 10];
        stopBtn = uicontrol('string',getString(message('MATLAB:funfun:optimplots:ButtonStop')),'Position',[stopBtnXYLoc 50 20],'callback',@buttonStop);
        ...
        pauseBtn = uicontrol('string',getString(message('MATLAB:funfun:optimplots:ButtonPause')),'Position',[60 10 50 20],'callback',{@buttonPauseContinue,fname});
...
for i = 1:length(plotNames)
    ...
    if isNew(i)
        ...
        uimenu(cmenu,'Label', getString(message('MATLAB:funfun:optimplots:LabelOpenPlotInNewWindow')), ...
...
elseif isempty(getappdata(gcf,'data'))
    ...
    set(hObj,'String',getString(message('MATLAB:funfun:optimplots:ButtonResume')));
else
    rmappdata(gcf,'data');
    return;
end
...
while true
    ...
    if isempty(getappdata(gcf,'data'))
        set(hObj,'String',getString(message('MATLAB:funfun:optimplots:ButtonPause')));
        return;
    end
...
msg = getString(message('MATLAB:funfun:optimplots:DialogStopSolverAndCloseFigure'));
handle = questdlg(msg,getString(message('MATLAB:funfun:optimplots:TitleCloseDialog')),...
 getString(message('MATLAB:funfun:optimplots:DialogYes')),...
 getString(message('MATLAB:funfun:optimplots:DialogNo')),...
 getString(message('MATLAB:funfun:optimplots:DialogNo')));
switch handle
    case getString(message('MATLAB:funfun:optimplots:DialogYes'))
        delete(obj)
    case getString(message('MATLAB:funfun:optimplots:DialogNo')) 
        return;
    otherwise
        return;
end
