...
%   $Revision: 1.1.6.9 $  $Date: 2013/10/15 02:53:33 $
    ...
    if ~isempty(arg)                      % [] is a valid options argument
        ...
        for j = 1:m
            ...
            if ~isempty(val)
                ...
                options.(Names{j,:}) = checkfield(Names{j,:},val,optimtbx);
            end
        end
    end
    i = i + 1;
...
while i <= numberargs
    ...
    if ~expectval
        ...
        if isempty(j)                       % if no matches
            ...
            stringWithLink = formatStringWithHyperlinks(getString(message('MATLAB:optimset:LinkToReferencePage')),'doc optimset');
        ...
        options.(Names{j,:}) = checkfield(Names{j,:},arg,optimtbx);
        expectval = 0;
    end
    i = i + 1;
end
...
switch lowerCaseOption
    case 'linesearchtype'
        removedOptExcept = MException('MATLAB:optimset:LStypeInvalid', ...
            '%s', getString(message('MATLAB:optimset:LStypeInvalid')));
end
