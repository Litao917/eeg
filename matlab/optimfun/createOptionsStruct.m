...
%   $Revision: 1.1.6.4 $  $Date: 2013/10/15 02:53:24 $
...
switch solverName
  ...
  case {'fminsearch','fzero','fminbnd','lsqnonneg'}
    ...
    case 'all'
        ...
        optionsFcn = '';
    otherwise
        error(message('MATLAB:createOptionsStruct:UnrecognizedSolver'));
end
...
for k = 1:length(deprecatedOpts)
    if isfield(userOptions,deprecatedOpts{k}) && ...
      ~isempty(userOptions.(deprecatedOpts{k}))
        ...
        if isempty(userSetOpts)  % Special format for 1st option in list
            userSetOpts = deprecatedOpts{k};
        else
            userSetOpts = [userSetOpts ', ' deprecatedOpts{k}];
        end
    end
end
...
if ~isempty(userSetOpts)
    msg = message('MATLAB:createOptionsStruct:DeprecatedOptions',userSetOpts);
