...
gadsSolvers = {'ga','patternsearch','gamultiobj','simulannealbnd','pso'};
...
if nargin == 1 && nargout <= 1 && isequal(solverName,'solvers')
    probStruct = struct( ...
        ...
        'fminbnd',[], 'fsolve',[],'lsqlin',[], ...
        'lsqnonneg',[], 'intlinprog', []);
...
if ~enableAllSolvers && any(strcmpi(solverName,gadsSolvers))
    warning(message('MATLAB:createProblemStruct:invalidSolver', upper( solverName ), upper( defaultSolver )));
...
switch solverName
    ...
    case 'all'
        ...
        solverName = defaultSolver;
   otherwise
        error(message('MATLAB:createProblemStruct:UnrecognizedSolver'));
end
