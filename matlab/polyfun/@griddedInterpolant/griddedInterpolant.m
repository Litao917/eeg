...
%
%    F = griddedInterpolant(...,METHOD, EXTRAPOLATIONMETHOD) specifies the 
%    extrapolation method.  EXTRAPOLATIONMETHOD has the same options as 
...
properties
    ...
    %ExtrapolationMethod - Defines the extrapolation method used to
    %    extrapolate queries outside of the domain defined by the grid vectors
    %    The Extrapolation method is one of the following: 
    %     'nearest' - nearest neighbor interpolation
    %     'linear'  - linear, bilinear, trilinear,... interpolation (default)
    %     'spline'  - spline interpolation
    %     'pchip'   - shape-preserving piecewise cubic interpolation (1D only)
