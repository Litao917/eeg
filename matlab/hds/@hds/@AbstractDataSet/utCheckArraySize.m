...
% Disallow defining dependent variables before grid is fully defined
% if any(GridSize==0) && isempty(locateGridVar(this,Variable))
%    error('Grid must be fully defined before specifying dependent data.')
% end
...
% Scalar expansion (except for grid variables)
if nas==1 && ngs>1 && isempty(GridDim)
   % RE: REPMAT does not like scalar size inputs
   ArrayValue = hdsReplicateArray(ArrayValue,[GridSize,1]);
   ArraySize = [GridSize,1];
   nas = ngs;
end
...
elseif ~isempty(GridDim)
   ...
   elseif nas==ngs
      ...
      if isnumeric(ArrayValue) && any(diff(sort(ArrayValue))==0)
         error('Duplicate grid points are not allowed.')
      end
   elseif nas~=sdim
      error(errmsg)
   end
...
if ngs==1 || nas==1
   ...
   if nddims>ndims && all(ArraySize(1:ndims)==1)
      % g191374
      ArraySize(:,1:ndims) = [];
   end
   SampleSize = [ArraySize , ones(1,2-length(ArraySize))];
   ArraySize = [1 1];
