...
GetAllVars = isequal(Vars,VarPool);
if GetAllVars
...
if GetAllVars
   % Optimization
   GridVarMap = this.Cache_.GridDim;
   GridVarMap(end+1:length(idxV),:) = 0;
else
   GridVarMap = utGridVarMap(this,Vars(idxV));
end
SectionSize = cellfun('length',GridSection);
ScalarSection = all(SectionSize==1);
...
for ctx=1:length(idxV)
   ...
   elseif length(GridSection)==1
      ...
      Slice = Containers(ct).getSlice(GridSection(gdim),'cell');
      if ~ScalarSection
         Slice = this.utGridFill(Slice,SectionSize,gdim);
      end
   end
   if isempty(Slice)
      fv{ct} = [];
