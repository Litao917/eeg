    ...
    if narg > 0
        ...
        elseif ~isempty(arg) && length(arg)==1 && ...
            ...
            if isa(handle(arg),'scribe.colorbar')
                ...
                colBar = arg;
                if nargin < 2
                    error(message('MATLAB:colorbar:TooFewInputs'));
                end
                args = args(2:end);
                narg = narg-1;
            else
...
%----------------------------------------------------------------------%
function tf = ishg2peer(varargin)
tf = false;
foundPeerString = false;
for i=1:numel(varargin)
    if strcmpi('peer',varargin{i})
        peerIndex = i;
        foundPeerString = true;
    end
end

if foundPeerString
    if (peerIndex < numel(varargin)) && (numel(varargin{peerIndex + 1})==1) && ishg2parent(varargin{peerIndex + 1})
            tf = true;
    end
end
