...
%   Copyright 1984-2011, 2013 The MathWorks, Inc.
%   $  $  $  $
...
if ~all(ishghandle(handles))
    error(message('MATLAB:selectobject:InvalidHandle'));
end
if nargin>1 && ...
        (~ischar(action) || ...
...
for i = 1:length(handles)
    ...
    if ~any(hMode.ModeStateData.ChangedObjectHandles == handle(handles(i)))
        if graphicsversion(handles(i),'handlegraphics')
            appdataObj = double(handles(i));
        else
            appdataObj = handles(i);
        end
...
for i = 1:length(handles)
    ...
    elseif strcmpi(get(handles(i),'tag'),'DataTipMarker') || strcmpi(get(handles(i),'tag'),'scribeOverlay')
        filteredIndices(i) = true;
    end
end
...
switch action
    case 'on'
        for i = 1:length(handles)
            ...
            if ishghandle(handles(i),'axes') && (graphicsversion(hFig,'handlegraphics') || ...
                  (~strcmpi(handles(i).Tag,'legend') && ~strcmpi(handles(i).Tag,'colorbar'))) 
...
if ~isempty(pm) && ishandle(pm)
    evdata = scribe.scribeevent(pm,'PlotSelectionChange');
...
%HG2 - Selected is not available (nor supported) on all objects. Hence
