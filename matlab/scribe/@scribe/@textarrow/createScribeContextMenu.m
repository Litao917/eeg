...
res = findall(hMenu,'Type','uimenu','-regexp','Tag','scribe.textarrow.uicontextmenu*');
if ~isempty(res)
    res = res(end:-1:1);
    return;
end
...
res(end+1) = graph2dhelper('createScribeUIMenuEntry',hFig,'Color',getString(message('MATLAB:uistring:scribemenu:TextBackgroundColorDotDotDot')),'TextBackgroundColor',getString(message('MATLAB:uistring:scribemenu:TextBackgroundColor')));
% Font:
...
for i=1:length(res)
    set(res(i),'Tag',['scribe.textarrow.uicontextmenu' '.' menuSpecificTags{i}]);
end
