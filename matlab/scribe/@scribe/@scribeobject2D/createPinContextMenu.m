...
res = findall(hFig,'Type','uimenu','-regexp','Tag','scribe.scribeobject2d.pinuicontextmenu*');
if ~isempty(res)
    res = res(end:-1:1);
    return;
end
...
res(end+1) = uimenu(hMenu,...
    ...
    'Visible','off','Tag','scribe.scribeobject2d.pinuicontextmenu.PinToAxes' ,...
    'Callback',{@localPinObject,hMode});
% "Unpin"
res(end+1) = uimenu(hMenu,...
    'HandleVisibility','off',...
    'Label',getString(message('MATLAB:uistring:scribemenu:Unpin')),...
    'Visible','off', 'Tag','scribe.scribeobject2d.pinuicontextmenu.Unpin' ,...
