...
if narg<2
	...
	if narg<1
		...
		if strcmp(arg1,'peer')
			error(message('MATLAB:colorbarv6:InvalidPeerAxes'));
		end
		loc = arg1;
	end
elseif narg == 2
	...
	if strcmp(arg1,'peer')
		if ishandle(arg2) & strcmp(get(arg2, 'type'), 'axes')
			...
			error(message('MATLAB:colorbarv6:SecondArgMustBeAxes'));
		end
	else
		error(message('MATLAB:colorbarv6:UnknownCommandOption'));
	end
else
	...
	if strcmp(arg2,'peer')
		if ishandle(arg3) & strcmp(get(arg3, 'type'), 'axes')
			...
			error(message('MATLAB:colorbarv6:ThirdArgMustBeAxes'));
		end
	else
		error(message('MATLAB:colorbarv6:UnknownCommandOption'));
	end
...
if narg==1
    if ishandle(loc)
        ...
        ud = get(ax,'userdata');
        if isfield(ud,'ColorbarDeleteProxy')
            error(message('MATLAB:colorbarv6:CannotAddToColorbar'))
        end
        if ~strcmp(get(ax,'type'),'axes')
            error(message('MATLAB:colorbarv6:AxesHandleRequired'));
        end
...
elseif loc(1)=='h', % create or refresh horizontal colorbar
  ...
  error(message('MATLAB:colorbarv6:HandleVertHorizExpected'))
end
