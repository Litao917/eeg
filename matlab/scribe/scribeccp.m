...
narg = nargin;
if (narg < 1 || narg > 3)
    error(message('MATLAB:scribeccp:InvalidNumberOfArgs'));
end
...
serialized = copyAxesRelationships(serialized, line);
...
if ~isempty(pm) && ishandle(pm)
    ...
    set(evdata,'ObjectsCreated',handle(newObjs),'Figure',fig);
...
if ~isempty(obj) && all(ishandle(obj))
    ...
    pasteAxesRelationships(serialized, obj);
end
