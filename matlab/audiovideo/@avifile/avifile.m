...
warning(message('MATLAB:audiovideo:avifile:FunctionToBeRemoved'));

if nargin == 0
  error(message('MATLAB:audiovideo:avifile:noFilenameProvided'));   
elseif ~isa(filename,'char')
  error(message('MATLAB:audiovideo:avifile:invalidFilenameArgument'));
end
...
if isunix
  ...
  if fid == -1
    error(message('MATLAB:audiovideo:avifile:errorCreatingTempFile', aviobj.TempDataFile));
  end
  fclose(fid);
  
  fid = fopen(aviobj.Filename,'a','l');
  if fid == -1
    error(message('MATLAB:audiovideo:avifile:errorCreatingAVI'));
  end
  fclose(fid);
end
...
if mmcompinfo('video','video 5.1') == -1
    ...
    warning(message('MATLAB:audiovideo:avifile:indeo5NotFound', privateGetInvalidCodecReference()));
    warning(bt);
    try
