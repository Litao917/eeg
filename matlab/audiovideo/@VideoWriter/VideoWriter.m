classdef VideoWriter < hgsetget & dynamicprops
    ...
    %                               - Explictly before the call to open OR
    ...
    methods
        function obj = VideoWriter(filename, profile)
            ...
            if ~ischar(filename)
                error(message('MATLAB:audiovideo:VideoWriter:invalidFilename'));
            end
            if isempty(filename)
                error(message('MATLAB:audiovideo:VideoWriter:emptyFilename'));
            end
            ...
            if (fid == -1)
                error(message('MATLAB:audiovideo:VideoWriter:fileNotWritable', filename, fidMessage));
            end
        ...
        function open(obj)
            ...
            try
                ...
                error(message('MATLAB:audiovideo:VideoWriter:fileNotWritable', obj.Filename, err.message));
            end
