...
warning(message('MATLAB:audiovideo:auread:functionToBeRemoved'));

if nargin>2,
  error(message('MATLAB:audiovideo:auread:tooManyInputs'));
end
...
if fid == -1,
  error(message('MATLAB:audiovideo:auread:unableToOpenFile'));
end
...
if strncmpi(ext,'size',exts),
  ...
  return;
elseif exts>2,
  error(message('MATLAB:audiovideo:auread:invalidIndexRange'));
elseif (exts==1),
...
if ~strcmp(snd.magic,'.snd'),
  error(message('MATLAB:audiovideo:auread:InvalidAUFile')); 
end
...
if cnt~=info_len,
  error(message('MATLAB:audiovideo:auread:errorReadingFile')); 
end
...
otherwise
    error(message('MATLAB:audiovideo:auread:invalidDataFormat')); 
end

% Determine # of samples per channel:
snd.samples = snd.databytes*8/snd.bits/snd.chans;
if snd.samples~=fix(snd.samples),
  error(message('MATLAB:audiovideo:auread:truncatedDataFile')); 
end
...
otherwise
  ...
  if isempty(ext),
    ...
    if numel(ext)~=2,
      error(message('MATLAB:audiovideo:auread:invalidLimitVector')); 
    end
    if ext(1)<1 || ext(2)>SamplesPerChannel,
      error(message('MATLAB:audiovideo:auread:sampleLimitsOutOfRange')); 
    end
    if ext(1)>ext(2),
      error(message('MATLAB:audiovideo:auread:sampleLimitsOutOfOrder'));
    end
  end
  ...
  if ext(1)>1,
    ...
    if status==-1,
      error(message('MATLAB:audiovideo:auread:invalidFileFormat'));
    end
  end
  % Read desired data:
