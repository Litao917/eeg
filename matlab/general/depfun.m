        ...
        while i <= nin
            if isa(varargin{i},'char')
                switch varargin{i}
                  ...
                  case '-nographics'
                    warning(message('MATLAB:DEPFUN:Deprecated', 'nographics'));
                  case '-alwayslog'
                    warning(message('MATLAB:DEPFUN:Deprecated', 'alwayslog'));
                  ...
                  case '-print'
                    if i < nin
                        ...
                        error(message('MATLAB:DEPFUN:NoPrintFile'));
                    end
                  case '-notrace'
                      warning(message('MATLAB:DEPFUN:DeprecatedNotrace'));
                  case '-expand'
                    expand = true;
                  case '-calltree'
                    calltree = true;
            ...
            elseif isa(varargin{i},'cell')
                ...
                error(message('MATLAB:DEPFUN:WrongClass', i));
            end
            i = i + 1;
        end

        if narg_files == 0
            error(message('MATLAB:DEPFUN:NoTraceArguments'));
        else
    ...
    function analyze_trace_all
        ...
        for i=1:newdepfun_nlhs
            switch i
                ...
                otherwise
                    error(message('MATLAB:DEPFUN:InternalError'));
            end
        end
    end
    %---------------------------------------------------------------------------
...
%---------------------------------------------------------------------------
    function output_report
        ...
        if fid < 0
          error(message('MATLAB:DEPFUN:CannotOpenFileForWrite', report_file));
        end
        ...
        if toponly
          toponly_msg = getString(message('MATLAB:DEPFUN:FormatTopOnly'));
        else
          toponly_msg = '';
        end 
        ...
        for i=1:noutput
          switch i
            case 1                % trace list
              ...
              %====================== Files not on MATLABPATH: 
              format = getString(message('MATLAB:DEPFUN:ReportFilesNotOnMATLABPATH'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
              if ~isempty(off_path)
              ...
              %====================== Handle Graphics factory callback names:
              if ~isempty(tmp_files)
                  format = getString(message('MATLAB:DEPFUN:ReportHandleGraphicsFactoryCallbackNames'));
                  fprintf(fid,format);
                  fprintf(fid,divider);                        % divider
            ...
            case 2                % builtin list
              %----------------------------------------------------------
              format = getString(message('MATLAB:DEPFUN:ReportBuiltinList')) ;
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
              if ~isempty(builtins)
            ...
            case 3                % MATLAB classes
              %----------------------------------------------------------
              format = getString(message('MATLAB:DEPFUN:ReportMATLABClasses'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
            ...
            case 4                % problem list
              %----------------------------------------------------------
              format = getString(message('MATLAB:DEPFUN:ReportProblemList'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
              if ~isempty(prob_files)
            ...
            case 5                % problem symbols: NOT IMPLEMENTED
              %----------------------------------------------------------
              format = getString(message('MATLAB:DEPFUN:ReportProblemSymbolsNOTIMPLEMENTED'));
              fprintf(fid,format);
              %----------------------------------------------------------
            case 6                % eval strings:
              %----------------------------------------------------------
              format = getString(message('MATLAB:DEPFUN:ReportEvalStrings'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
            ...
            case 7                % called from / call list
              if ~calltree
                  %----------------------------------------------------------
                  format = getString(message('MATLAB:DEPFUN:ReportCalledFromListbyTraceList'));
                  fprintf(fid,format);
                  fprintf(fid,divider);                        % divider
                  ...
                  %----------------------------------------------------------
                  format = getString(message('MATLAB:DEPFUN:ReportCallListbyTraceList'));
                  fprintf(fid,format);
                  fprintf(fid,divider);                        % divider
            ...
            case 8                % opaque classes
              %----------------------------------------------------------
              format = getString(message('MATLAB:DEPFUN:ReportOpaqueClasses'));
              fprintf(fid,format);
              fprintf(fid,divider);                        % divider
        ...
        for i=1:noutput
          switch i
            case 1
              ...
              data{n  ,2} = [];
              if toponly
                data{n+1,2} = getString(message('MATLAB:DEPFUN:FormatTopOnly'));
              else
                data{n+1,2} = '';
              end 
              data{n+2,2} = [];
    ...
    function [cbnames,cbstrings] = analyze_fig_file(figfile,k)
        ...
        fn = fieldnames(data);
        if length(fn) > 1
            warning(message('MATLAB:DEPFUN:BadHGStructure', figfile));
            return
        end
        hg_struct = data.(fn{1});
        ...
        if k==1
            if ~quiet
                format = getString(message('MATLAB:DEPFUN:ReportGeneratingHandleGraphicsCallbackNamesToAnalyzefigFil'));
                fprintf(format);
            end
            hg_cbnames = create_hg_cbnames;
            if ~quiet
                format = getString(message('MATLAB:DEPFUN:ReportDone'));
                fprintf(format);
            end
        end
    ...
    for j=1:nfield
        ...
        if any(strcmp(hg_cbnames,lstructnames{j}))
            if ischar(fi)
                ...
                cbstrings{end+1} = fi;
            elseif ~isempty(fi)
                warning(message('MATLAB:DEPFUN:NotAStringForCallback', strtrim( lstructnames{ j } )));
            end
        elseif isstruct(fi)
...
function [trace_file,off] = next_arg_file(arg_file)
    ...
    error(message('MATLAB:DEPFUN:FileDoesNotExist', arg_file));
end
...
function fn = stdpath(file,root)
    ...
    if ndirs < 0
        error(message('MATLAB:DEPFUN:PathBeyondRoot', file));
    end
end
