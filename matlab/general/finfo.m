...
if ~isempty(ext)
    ...
    elseif isempty(openAction) % open handler
        [ext, description] = getFileInfo(filename, ext);
...
avWarnState(end+1) = warning('OFF', 'MATLAB:audiovideo:wavfinfo:functionToBeRemoved');
avWarnState(end+1) = warning('OFF', 'MATLAB:audiovideo:aufinfo:functionToBeRemoved');
...
function [ext, description] = getFileInfo(filename, fileextension)
