...
try
    % get the primary userpath in case it has been removed from the path
    workpath = [ system_dependent('getuserworkfolder') ps ];

    % get the full userpath so it won't be inlined in the generated file
    fulluserpath = regexp(userpath(), DelimitedByAndIncludingSep, 'tokens');
    fulluserpath = [fulluserpath{:}]';
    
    % exclude case-insensitively from path list that will be saved
    pathmatch = strcmpi(all_path_lines, workpath);
    
    % If the primary userpath is not found, then the user deleted 
    % it from the path.
    if ~any(pathmatch)
        userpath('clear')  %clear the userpath setting
    end
    
    % find the index for any folder in the full user path
    for idx = 1 : numel( fulluserpath )
        pathmatch = pathmatch | strcmpi( all_path_lines, fulluserpath{idx});
    end
