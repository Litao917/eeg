classdef ComponentInfo < handle
    ...
    methods
        ...
        function r = ComponentDirectory(ci, component)
            r = getComponentData(ci, component, 'ComponentDirectory');
            r = ci.ixf.bind(r);
        end
    end
end
...
function ProcessSearchPathNode(component, node, info)
    pth = char(node.getAttribute('component'));
    contentsDir = char(node.getAttribute('contentsDir'));
    if isKey(info, component)
        data = info(component);
    end
 
    data.ComponentDirectory = pth;
