...
%   * Explicit dependencies
    ...
    properties (Constant)
        ToolboxRoot = strrep(fullfile(matlabroot, 'toolbox'),'\','/');
        MatlabRoot = strrep(matlabroot,'\','/');
        
        keywords = { ...
            'target', 'include', 'exclude', 'license', ...
            'insert', 'expect', 'allow', 'remove', 'move' ...
        };
    ...
    methods(Access = public)
        ...
        function addRules(obj, file)
            ...
            if rds == 0
                ...
                error(message(...
                    'MATLAB:Completion:InternalCreateRulesDirFail',...
                    ruleFcnDir));
            end
            ...
            if e ~= 2
                error(message(...
                    'MATLAB:Completion:InternalRulesFileMustExist', ...
                    pth));
            end
            ...
            if e ~= 2
                error(message(...
                    'MATLAB:Completion:InternalRulesFileMustRun', ...
                    fcn, e));
            end
            ...
            % Actually delete this temporary file and the directory
            % that contains it.
            delete(pth);
            rmdir(ruleFcnDir);
    ...
    methods(Access = private)
        ...
        function initProperties(obj)
            ...
            obj.excludeList = matlab.depfun.internal.WhyList(...
                'MATLAB:Completion:InternalNoExclusions');

            obj.expectList = matlab.depfun.internal.WhyList(...
                'MATLAB:Completion:InternalNoExpected');
        ...
        function tbx = whichToolboxOwns(obj, file)
            ...
            % <matlabroot>/toolbox.
            match = regexp(file, [obj.ToolboxRoot '/([^/]+)/'], 'tokens');
            if ~isempty(match)
                tbx = match{1}{1};
            end
        end
        ...
        function [fcn, pth] = codify(obj, ruleFcnDir, file)
            ...
            %TODO: Hack name to help MCC's MCR find it. Name must exist as an
            % M-file on the MATLAB path before MCC starts. What a PIA!
            ...
            if ffp < 0
                error(message(...
                    'MATLAB:Completion:InternalCreateRulesFileFail',...
                    pth));
            end
        ...
        function msg = formatExcludeMsg(obj, target)
            msg = msg2why(message('MATLAB:Completion:ExcludedBy', ...
                                    target, obj.currentLicense));
        end
        
        % Construct a well-formatted exclude message.
        function msg = formatRequireMsg(obj, target)
            msg = msg2why(message('MATLAB:Completion:ExpectedBy', ...
                                    target, obj.currentLicense));
        end
    ...
    methods (Access = public)
        ...
        % any regexp characters in the literal strings.
            if nargin ~= 4
                error(message(...
                      'MATLAB:Completion:RDLAllowBadArgCount', ...
            ...
            tgtMap(setName) = [ tgtMap(setName) literal ];            
        end
        ...
        % Allow Add file patterns to the allow list for a given target
            ...
            tgtMap(setName) = [ tgtMap(setName) list ];
        end
        ...
        % REMOVE Create rules that remove files from a named set.
            if nargin ~= 4
                error(message(...
                      'MATLAB:Completion:RDLRemoveBadArgCount', ...
                      obj.activeRulesFile, obj.activeRulesLine, nargin));
            end
            ...
            for n=1:numel(list)
                ...
                if isempty(m)
                    error(message('MATLAB:Completion:RDLBadlyFormedMoveRule',...
        ...
        % MOVE Create rules that move files between named sets
            if nargin ~= 5
                error(message(...
                      'MATLAB:Completion:RDLMoveBadArgCount', ...
                      obj.activeRulesFile, obj.activeRulesLine, nargin));
            end
        ...
        % INSERT Create rules that add files to a named set.
            if nargin ~= 4
                error(message(...
                      'MATLAB:Completion:RDLInsertBadArgCount', ...
                      obj.activeRulesFile, obj.activeRulesLine, nargin));
            end
        ...
        % interesting things later on.)
            switch nargin
                ...
                otherwise
                  error(message(...
                      'MATLAB:Completion:RDLBadLicense', nargin));
            end
        end
