    ...
    properties (Access = private)
     ...
     methods
        ...
        % origin and destination must be the same length.
            ...
            [~,ia] = intersect(origin, original);
            vertexID = vertexID(ia);
        ...
        % file does not exist in the manifest.
            ...
            if nargin ~= 2
                error(message('MATLAB:Completion:BadInputCount', 2, nargin, ...
                              'requirements'));
            end
...
function keep = symbolFilter(symbol)
    % Filter out built-ins, as they have no file.
    keep = arrayfun(@(s)~isBuiltin(s), symbol);
end
