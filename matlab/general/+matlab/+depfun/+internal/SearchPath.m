    ...
    methods (Access = private)
        ...
        % Return d's index on the PathList.
            ...
            if any(idx)
                r = find(idx);
            elseif mustExist
                error(message('MATLAB:Completion:NonExistSearchDir', d));
            end
        end
        ...
        % items and remove the exclude items.
            ...
            if ~isequal(obj.Components, ...
                ...
                match = ...
                    regexp(pth, ['toolbox\' filesep '([^\' filesep ']+)' ...
                                 '(\' filesep '|$)']);
                ...
                tbxML = regexp(pth, ['toolbox\' filesep ...
                           '((matlab\' filesep ')|(local[\' filesep ']?))']);
            ...
            % PathList.
            if ~isempty(path_limit)
                path_limit = strrep(path_limit, filesep, '/');
            ...
            obj.PathList = strrep(obj.PathList, '/', filesep);
        ...
        % duplicates leak in, ASSEMBLE_PATH is likely to fail.)
            ...
            if ~isempty(obj.Target) && ...
                ...
                tgtList = make_delimited_list(obj.KnownTargets, ', ');
                error(message('MATLAB:Completion:TargetInfoNotInDB', ...
    ...
    methods
        ...
        %   tbxData.db as the database.
                ...
                switch ps
                    case '-p'
                        if idx+1 > numel(varargin)
                            error(message(...
                                'MATLAB:Completion:MissingPathArg', ...
                                idx, varargin{idx}));
                        end
                        idx = idx + 1;
                    ...
                    case '-i'
                        ...
                        if idx <= numel(varargin)
                            ...
                            error(message(...
                             'MATLAB:Completion:InvalidSearchDirectory', ''));
                        end
                    case '+i'
                        idx = idx + 1;
                        if idx <= numel(varargin)
            ...
            if isfield(s.IncludePath, 'atStart') ...
                ...
                error(message('MATLAB:Completion:InvalidIncludePathType', ...
            ...
            % Database must exist.
            if exist(s.Database, 'file') ~= 2
              error(message('MATLAB:Completion:NonExistDatabase', ...
                                      s.Database));
            end
            ...
            if isempty(s.Components) || ...
               (iscell(s.Components) && ...
                ...
                error(...
                  message('MATLAB:Completion:InvalidSearchComponent',''));
            end
        ...
        % directory we're adding already exists in the path list, remove the
        % existing directory before adding the new one. This allows ADD() to
        % change the order of the search path.
            ...
            if ~iscell(directoryList)
                error(message('MATLAB:Completion:InvalidInputType', 2, ...
            ...
            function add_dir(d, atEnd)
                if exist(d, 'dir') == 7
                    ...
                    error(message('MATLAB:Completion:NonExistDir', d));
                end
            end
        ...
        % lexicographic order.
            ...
            if ~islogical(flag)
                error(message(...
                        'MATLAB:Completion:IllogicalSortProperty', ...
                        flag, class(flag)));
            end       
        ...
        % component list. 
            ...
            if isempty(pathItems)
                error(message('MATLAB:Completion:InvalidSearchDirectory', ...
                          ''));  
            end
            ...
            elseif ~iscell(pathItems)
                error(message('MATLAB:Completion:InvalidPathLimitType', ...
                              class(pathItems)));
            else
        ...
        % the directory under $MATLAB/toolbox containing the toolbox.
            ...
            if isempty(componentList)
                error(message('MATLAB:Completion:InvalidSearchComponent', ...
                          ''));
            end
        ...
        % KnownTargets is set in the constructor.
            if matlab.depfun.internal.Target.parse(target) == ...
                ...
                tgtList = make_delimited_list(obj.KnownTargets, ', '); %#ok
                error(message('MATLAB:Completion:InvalidTarget', ...
                              target, tgtList));                
            end
            if ~any(ismember(obj.KnownTargets, target)) %#ok
                tgtList = make_delimited_list(obj.KnownTargets, ', '); %#ok 
                error(message('MATLAB:Completion:TargetInfoNotInDB', ...
...
function str = make_delimited_list(cellData, delimiter)
% Make a delimited list from the cell array of strings.
    str = '';
    if ~isempty(cellData)
        list = cellfun(@(t)sprintf('%s%s', delimiter, t), ...
                       cellData(2:end), 'UniformOutput', false); 
        str = [ cellData{1} list{:} ];
    end
end
...
% corresponding full path. partial is a cell array of strings.
    ...
    function path_item = parent_to_toolbox(d)
        path_item = d;
        tbx_path = fullfile(matlabroot, 'toolbox', d);
    ...
    % paths.
    path_list = cellfun(@parent_to_toolbox, partial, ...
                        'UniformOutput', false);
...
% matched.
    ...
    path_items = cellfun(@(d) ...
         regexp(path,['(^|' pathsep ')[^' pathsep ']*' d '(' pathsep '|$)'],...
                'match','once'), ...
    ...
    % point.
    path_items = strrep(path_items,pathsep,'');
...
% those parents and all their children which are on the MATLAB path.
    ...
    function kids = find_children(p)
        ...
        found = regexp(pth, [p '\' filesep],'once');
...
% Connect to the given database using the SqlDbConnector
    if exist(file, 'file') ~= 2
        error(message('MATLAB:Completion:NonExistDatabase', file));
    end
