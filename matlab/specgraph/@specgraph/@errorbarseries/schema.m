...
%AII fixes geck 452490, now use 'lineMarkerFaceColorType' instead of 'MATLAB
%array' type to get the right renderer and editor
...
hProp = schema.prop(hClass,'ParentListener','handle');
hProp.Visible = 'off';
hProp.AccessFlags.Serialize = 'off';
...
function LdoUpdateAction(hSrc, eventData) %#ok
h = eventData.affectedObject;
if h.initialized
...
function LdoUpdateXDataAction(hSrc, eventData) %#ok
h = eventData.affectedObject;
