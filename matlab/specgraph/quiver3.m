...
if ishg2parent( varargin{:} )
    ...
    try 
        h = quiver3HGUsingMATLABClasses(cax, args{:});
    catch me
        throw(me)
    end
else
    ...
    % old implementation
    if v6
        h = Lquiver3v6(cax, args{:});
        
        % create quiver object
    else
        nargs = length(args);
        error(nargchk(4,inf,nargs,'struct'));
...
%----------------------------------------------%
function hh = Lquiver3v6(cax, varargin)
args = varargin;
nargs = length(args);
...
while ischar(args{nin}),
    ...
    if ~isempty(vv) && strcmpi(vv(1),'f')
        ...
        if ~isempty(msg),
            error(message('MATLAB:quiver3:UnknownOption', vv));
        end
...
if ~isequal(size(x),size(y),size(z),size(u),size(v),size(w))
    error(message('MATLAB:quiver3:DataSizeMismatch'));
end
...
if length(autoscale)>1,
    error(message('MATLAB:quiver3:NonScalarFactor'));
end
