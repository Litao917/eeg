...
function [pvpairs, args, nargs, msg] = localParseargs(varargin)
    ...
    % We have nans in XData/YData working for line contours, but not 
    % for filled contours. The bmin field happens to be true for filled 
    % contours.
    allow_xy_nans = ~bmin;
    ...
    if nargs > 0 && ischar(args{end})
        ...
        if ~isempty(tmsg)
            msg = getString(message('MATLAB:uistring:contourobjhelper:UnknownOption', args{end}));
        end
        if ~isempty(lColor)
