...
if ishg2parent( varargin{:} )
    ...
    try 
        h = scatter3HGUsingMATLABClasses(cax, args{:});
    catch me
        throw(me)
    end
else
...
%--------------------------------------------------
function [hh] = Lscatterv6(cax, varargin)
...
elseif isequal(size(c),[length(x) 3]), % vector of rgb's
    color = c;
     ...
     error(message('MATLAB:scatter3:invalidCData'))
end
...
elseif length(s)~= numel(s) || length(s)~=length(x)
    error(message('MATLAB:scatter3:invalidSData'))
end
...
if any([length(x) length(y) length(z) ...
    ...
    msg = message('MATLAB:scatter3:invalidData');
end
...
elseif isequal(size(c),[length(x) 3]), 
    ...
    msg = message('MATLAB:scatter3:invalidCData');
end
...
if length(s) > 1 && ...
    ...
    msg = message('MATLAB:scatter3:invalidSData');
end
