function hout = contourslice(varargin)
    ...
    if ishg2parent( varargin{:} )
        try
            h = contoursliceHGUsingMATLABClasses(varargin{:});
        catch me
            throw(me)
        end
    else
...
function [x, y, z, data, sx, sy, sz, cvals, method] = parseargs(nin, vargin)
    ...
    if nin > 0
        lastarg = vargin{nin};
        if ischar(lastarg) % contourslice(..., 'method')
            method = lastarg;
            nin = nin - 1;
        end
    end
    ...
    elseif nin == 8     % contourslice(x, y, z, v, sx, sy, sz, cvals)
        ...
        error(message('MATLAB:contourslice:WrongNumberOfInputs'));
    end
end
