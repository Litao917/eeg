...
if nargin > 2 || (nargin ==1 && ~ischar(a))
    error(message('MATLAB:colstyle:InvalidInput'));
end
...
if any(all(aa~=b)) 
  msg.identifier = 'MATLAB:colstyle:UnrecognizedLinespecCharacter';
...
if length(style)+length(color)+length(marker) ~= length(a)
  msg.identifier = 'MATLAB:colstyle:InvalidLinespec';
  msg.message = getString(message(msg.identifier));
end

if nargin == 2 && strcmp(plotout,'plot')
