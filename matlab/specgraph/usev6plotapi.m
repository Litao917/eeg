    ...
    if narg>0 && isa(args{1},'char')
        ...
        elseif (narg == 1) && strcmp(args{1},'on')
            if isempty(getappdata(0,'UseV6PlotAPI'))
                warning(message('MATLAB:usev6plotapi:DeprecatedMode'))
...
if isempty(filename)
    warning(message('MATLAB:usev6plotapi:DeprecatedV6Argument'));
else
    warning(message('MATLAB:usev6plotapi:DeprecatedV6ArgumentForFilename', upper(filename)));
end
