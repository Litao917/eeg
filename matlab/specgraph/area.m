function hh = area(varargin)
    ...
    %   Copyright 1984-2010, 2012 The MathWorks, Inc.
...
function h = Lareav6(cax, varargin)
    args = varargin;
    nargs = length(args);
    ax = newplot(cax);
    ...
    if nargs<1,
        error(message('MATLAB:area:NotEnoughInputs'));
    ...
    if ~isempty(msg), error(msg); end
    if all(size(level))~=1,
        error(message('MATLAB:area:LevelMustBeScalar'));
    end
...
function [args,pvpairs,msg] = parseargs(args)
    ...
    nargs = length(args);
    level = [];
    if (nargs==2) % area(x,y) or area(y,level)
        % area(y,level)
        if ~isequal(size(args{1}),size(args{2})) && ...
                length(args{2})==1,
          level = args{2};
        end
    elseif (nargs==3) % area(x,y,level)
      level = args{3};
    end
    
    if ~isempty(level)
      if ~isscalar(level) 
        msg = message('MATLAB:area:LevelMustBeScalar');
      elseif ~isfinite(level) 
        msg = message('MATLAB:area:LevelMustBeScalar');
      else
    ...
    if isempty(args)
        msg = message('MATLAB:area:NoDataInputs');
    else
