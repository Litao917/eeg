function [cout, hand, cf] = contourf(varargin)
    ...
    %    fills the regions between the contour levels. The z-value of the
    %    region's lower contour level determines the color from the
    %    colormap used to fill that contour region.
    %
    ...
    if ishg2parent( varargin{:} )
        ...
        if v6 || (nout == 3)
            if (nout == 3)
                warning(message('MATLAB:contourf:DeprecatedV6OutputArgument', upper( mfilename )));
            end
            [c, h, cs] = Lcontourfv6(cax, args{:});
        else
...
function [c, h, CS] = Lcontourfv6(cax, varargin)
    % Parse possible Axes input
    args = varargin;
    nargs = length(args);
    ...
    for i = 1 : nargs
        if isempty(args{i})
            error(message('MATLAB:contourf:EmptyInput'));
        end
    end
    ...
    maxz = max(z(i));
    if ~any(i)
        error(message('MATLAB:contourf:NonFiniteData'));
    elseif isempty(z) || (maxz == minz)
        error(message('MATLAB:contourf:ConstantData'));
    end
...
function [pvpairs, args, msg] = parseargs(args)
    ...
    if ~isempty(z) && isempty(msg)
        ...
        zmin = min(z(k));
        if ~any(k)
            warning(message('MATLAB:contourf:NonFiniteData'));
        elseif isempty(z) || (zmax == zmin)
            warning(message('MATLAB:contourf:ConstantData'));
        end
    end
    args = [];
end
