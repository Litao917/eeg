function fvcout = isocolors(varargin)
    ...
    if ~isempty(c)
        [msg, x, y, z] = xyzrgbcheck(x,y,z,c);
        if ~isempty(msg),
            error(msg);
        end
    else
        [msg, x, y, z] = xyzrgbcheck(x,y,z,r,g,b);
        if ~isempty(msg),
            error(msg);
        end
    end
    ...
    if isscalar(verts) && ishghandle(verts)
        if strcmp(get(verts, 'type'), 'patch')
            ...
            error(message('MATLAB:isocolors:InvalidPatchHandle'));
        end
    else
        p = [];
    end
...
function [x, y, z, data, r,g,b,verts] = parseargs(nin, vargin)
    ...
    elseif nin==7  % isocolors(x,y,z,r,g,b, verts/p)
        ...
        error(message('MATLAB:isocolors:WrongNumberOfInputs'));
    end
end
...
function [msg,nx,ny,nz] = xyzrgbcheck(x,y,z,r,g,b)
    ...
    if nargin>4
        if ~isequal(size(g), sz) || ~isequal(size(b), sz)
            msg = message('MATLAB:isocolors:RGBSizeMismatch');
            return
        end
    end
    
    if ndims(r)~=3
        msg = message('MATLAB:isocolors:InvalidColorDimensions');
        return
    end
    if min(sz)<2
        msg = message('MATLAB:isocolors:InvalidColorSizes');
        return
    end
    
    nonempty = ~[isempty(x) isempty(y) isempty(z)];
    if any(nonempty) && ~all(nonempty)
        msg = message('MATLAB:isocolors:InputsEmptyMismatch');
        return;
    end
    ...
    if ~isempty(nx) && ~isequal(size(nx), sz)
        nx = nx(:);
        if length(nx)~=sz(2)
            msg = message('MATLAB:isocolors:XSizeMismatch');
            return
        else
    ...
    if ~isempty(ny) && ~isequal(size(ny), sz)
        ny = ny(:);
        if length(ny)~=sz(1)
            msg = message('MATLAB:isocolors:YSizeMismatch');
            return
        else
    ...
    if ~isempty(nz) && ~isequal(size(nz), sz)
        nz = nz(:);
        if length(nz)~=sz(3)
            msg = message('MATLAB:isocolors:ZSizeMismatch');
            return
        else
