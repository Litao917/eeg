...
if ishg2parent( varargin{:} )
    ...
    try 
        h = scatterHGUsingMATLABClasses(cax, args{:});
    catch me
        throw(me)
    end
else
...
function h = Lscatterv6(cax, varargin)
args = varargin;
nargs = length(args);
...
otherwise
    error(message('MATLAB:scatter:InvalidInput'));
end

if length(x) ~= length(y) || ...
        length(x) ~= numel(x) || length(y) ~= numel(y)
    error(message('MATLAB:scatter:InvalidData'));
end
...
elseif length(s)~=numel(s) || length(s)~=length(x)
    error(message('MATLAB:scatter:InvalidSData'))
end
...
if isempty(args)
  msg = message('MATLAB:scatter:NoDataInputs');
else
