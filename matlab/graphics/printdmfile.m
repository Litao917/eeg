...
if (feature('HGUsingMATLABClasses') == 1)
    error(msgID, getString(message('MATLAB:print:DeprecatedMATLABCodeGenerationOption')));
else
    warning(msgID, getString(message('MATLAB:printdmfile:DeprecatedMATLABCodeGenerationOption')));
end
...
if ~isempty(mat)
   ...
   if isfield(options, 'showMessageForMATFile') & options.showMessageForMATFile ==1 
       warning(message('MATLAB:printdmfile:KeepFilesTogether', Mat_filename, M_filename)); 
   end 
end
...
if ~ishandle(handle)
    error(message('MATLAB:printdmfile:InvalidHandle'))
end


% Check if 'handle' refers to a figure
if ~strcmp(get(handle,'type'),'figure')
    error(message('MATLAB:printdmfile:InvalidFigureHandle'))
end
...
if strncmpi('graph',Type,5)
    if isempty(gcbo)
        disp(getString(message('MATLAB:printdmfile:ObjectsAreNotSupported',Type)))
        disp(getString(message('MATLAB:printdmfile:Skipping')))
    else
        warndlg(getString(message('MATLAB:printdmfile:DialogObjectsAreNotSupported',Type)));
    end
    SUP = 0;
end
...
elseif isstruct(value) 
    ...
    if ~found
        error(message('MATLAB:printdmfile:unsupportedDataType')); 
    end
end
...
dim =size(value); 
if (length(dim)>2) 
    error(message('MATLAB:printdmfile:unableToGenerateMCodeForChar')); 
end 
...
if (length(dim)>2) 
    error(message('MATLAB:printdmfile:unableToGenerateMCodeForCell')); 
end 
...
if fid < 1
    error(message('MATLAB:printdmfile:errorWritingFile'))
end
