...
FF = graphics.internal.figfile.FigFile;
FF.Path = filename;
FF.MatVersion = SaveFlag;
FF.FigFormat = 2;
        
% Warn for handle graphic and error for MATLAB Graphics
if SaveAll
...
if graphics.internal.graphicsversionWithEmpty(h, 'handlegraphics') 
    ...
    % new Figure window to be displayed. This is a change in behavior from HG1 since in HG1 only the double
    % handle would have been saved into a MAT File.
...
[hInput, fileInput] = graphics.internal.figfile.processSaveArguments(handle_file{:});
