...
function ui=loadsizepanel(style,ui)
  ...
  [vals,strs] = getUnitsVals; %#ok<NASGU>
...
function loadrenderingpanel(style,ui)
  ...
  color = style.Color;
  [colors,strs] = getColorVals; %#ok<NASGU>
...
function loadfontpanel(style,ui)
  ...
  [vals,strs] = getWeightVals; %#ok<NASGU>
  ...
  [vals,strs] = getAngleVals; %#ok<NASGU>
...
function doRestore(hSrc, eventData,fig) %#ok<INUSL>
  ...
  if ~isempty(ui.figure) && ishghandle(ui.figure) && ~isempty(ui.oldstate)
      ...
      for n=1:length(old.objs)         
          % Values and property names must be of the same datatypes
          % If property names are a cell make values a cell array
          if ~iscell(old.values{n}) && iscell(old.prop{n})
              old.values{n} = {old.values{n}}; %#ok<CCAT1>
          end
          % If values are a cell make property names a cell array
          if ~iscell(old.prop{n}) && iscell(old.values{n})
              old.prop{n} = {old.prop{n}}; %#ok<CCAT1>
          end
          % Assert that properties and values are the both cells.  
          if iscell(old.prop{n}) || iscell(old.values{n})
              assert(strcmpi(class(old.prop{n}), class(old.values{n})));
          end
                
          % Safe guard against sets on invalid handles. See g873428
          hObj = old.objs{n};          
          validHandles = ishghandle(hObj);
...
function rendererChanged(hSrc, eventData,fig) %#ok<INUSL>
  ...
  ui = getui(fig);
  if ui.active
    [rends,strs] = getRendererVals; %#ok<NASGU>
...
function [out,strs] = getRendererVals
  if matlab.graphics.internal.isGraphicsVersion1
    ...
    strs = {getString(message('MATLAB:exportsetupdlg:PaintersDropdown')),getString(message('MATLAB:exportsetupdlg:OpenGLDropdown')),...
              getString(message('MATLAB:exportsetupdlg:ZbufferDropdown'))};
  else
    out = {'painters','opengl'};
...
function [out,strs] = getWeightVals
  if matlab.graphics.internal.isGraphicsVersion1
    ...
    strs =  {getString(message('MATLAB:exportsetupdlg:NormalDropdown')),...
        getString(message('MATLAB:exportsetupdlg:LightDropdown')),...
        getString(message('MATLAB:exportsetupdlg:DemiDropdown')),...
...
% % Creates a cellarray with translated values.
