...
set(p,'FactoryValue',get(0,'DefaultTextFontName'));
p = schema.prop(cls, 'FontSize','double');

% We'd like the tooltip text size to be slightly smaller than the default
% font size. But if the default font size is less than ten, the tool tip is
% often too small. So when calculating the tool tip font size, use the max
% of 10 and the DefaultTextFontSize.

fontSize = max(get(0,'DefaultTextFontSize'), 10);
set(p,'FactoryValue',round(fontSize*(4/5)));
if ismac
    set(p,'FactoryValue',round(fontSize*(9/10)));
end
...
p = schema.prop(cls, 'PointsOffset','MATLAB array');
p.Visible = 'off';
p.FactoryValue = [8,8];
...
p = schema.prop(cls, 'EventObject', 'handle');
p.Visible = 'off';
p.AccessFlags.PublicSet = 'off';
p.AccessFlags.PublicGet = 'off';

p = schema.prop(cls, 'DataManagerHandle', 'handle');
p.Visible = 'off';
p.FactoryValue = [];
