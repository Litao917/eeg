...
% Constructor
hThis = graphics.datatip('Parent',hAxes,'Serializable', 'off');
...
hAxes = getaxes(hThis);
if isempty(hAxes)
    error(message('MATLAB:graphics:datatip:emptyAxes'))
end
...
hHost = hThis.Host;
if isempty(hHost) || ~ishandle(hHost)
    % Remove any listeners on the current host
    hThis.HostListenerHandles = [];
...
% Get the current position and units.  We will want to set the units back
% at the end and we may need the Z value of the position to restore correct
% Z-stacking
setappdata(hThis.HostAxes,'datatip_fireDataTipUpdate',false);
orig_units = get(hText, 'Units');
orig_pos = get(hText, 'Position');
...
elseif strcmp(orientation,'top-right')
    ...
    error(message('MATLAB:graphics:datatip:invalidOrientation'))
end
...
%-------------------------------------------------%
function localSetPosition(~,evd,hThis)
...
if isempty(new_position)
    ...
    error(message('MATLAB:datatip:EmptyPosition'));    
end
