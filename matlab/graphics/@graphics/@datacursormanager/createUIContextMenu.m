...
props.Label = getString(message('MATLAB:uistring:datacursor:MenuSelectionStyle'));
props.Separator = 'off';
...
props.Label = getString(message('MATLAB:uistring:datacursor:MenuMousePosition'));
props.Separator = 'off';
...
props = [];
props.Parent = s.cursor_style;
props.Label = getString(message('MATLAB:uistring:datacursor:MenuSnapToNearestDataVertex'));
props.Separator = 'off';
...
props = [];
props.Parent = s.main;
props.Label = getString(message('MATLAB:uistring:datacursor:MenuDisplayStyle'));
props.Separator = 'off';
...
props.Label = getString(message('MATLAB:uistring:datacursor:MenuWindowInsideFigure'));
props.Separator = 'off';
...
props = [];
props.Parent = s.disp_style;
props.Label = getString(message('MATLAB:uistring:datacursor:MenuDatatip'));
props.Separator = 'off';
...
props = [];
props.Parent = hContextMenu;
props.Label = getString(message('MATLAB:uistring:datacursor:MenuCreateNewDatatip')); 
props.Separator = 'on';
...
props.Parent = hContextMenu;
props.Label = getString(message('MATLAB:uistring:datacursor:MenuDeleteCurrentDatati')); 
props.Separator = 'off';
...
props.Parent = hContextMenu;
props.Label = getString(message('MATLAB:uistring:datacursor:MenuDeleteAllDatatips')); 
props.Separator = 'off';
...
% export
props.Parent = hContextMenu;
props.Label = getString(message('MATLAB:uistring:datacursor:MenuExportCursorDataToWorkspace'));
props.Separator = 'on';
...
if ~isdeployed
    ...
    props.Parent = hContextMenu;
    props.Label = getString(message('MATLAB:uistring:datacursor:MenuEditTextUpdateFunction'));
    props.Separator = 'on';
    ...
    props.Parent = hContextMenu;
    props.Label = getString(message('MATLAB:uistring:datacursor:MenuSelectTextUpdateFunction'));
...
elseif isequal(obj,s.export)
   ...
   while exists && ~userAns
        warnMessage = getString(message('MATLAB:uistring:datacursor:WarningVariableExists',...
            answer{1},answer{1}));
       userAns = localUIPrefDiag(hFig, warnMessage, getString(message('MATLAB:uistring:datacursor:TitleExportCursorDataToWorkspace')),'DataCursorVariable');
       if ~userAns
           answer=inputdlg(prompt,name,numlines,defaultanswer);
           if ~isempty(answer) && ischar(answer{1})
               exists = evalin('base', ['exist(''' answer{1} ''',''var'')']);
   ...
   if ~isempty(answer) && ischar(answer{1})
       ...
       try
           ...
           if strcmpi(id,'MATLAB:assigninInvalidVariable')
               errordlg(getString(message('MATLAB:uistring:datacursor:DialogInvalidVariableName',answer{1})),...
                   getString(message('MATLAB:uistring:datacursor:DialogCursorDataExportError')));
           else
               errordlg(getString(message('MATLAB:uistring:datacursor:DialogErrorWhileSavingData')),...
                   getString(message('MATLAB:uistring:datacursor:DialogCursorDataExportError')));
           end
       end
   end
...
res = edtMethod('showOptionalConfirmDialog','com.mathworks.widgets.Dialogs',canvases,...
    theMessage,title,com.mathworks.mwswing.MJOptionPane.CANCEL_OPTION,...
