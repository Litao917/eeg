...
fclose(rsp_fid);
if pj.DebugMode
    disp( sprintf('%s',getString(message('MATLAB:ghostscript:PRINTDebuggingCallGscriptWith', rsp_file, pj.FileName ))) )
end 
s = 0;
r = '';
...
if pj.DebugMode
    disp(getString(message('MATLAB:ghostscript:PRINTDebuggingGHOSTSCRIPTConvertingPSFile', pj.FileName)));
...
if s && ~isempty(r)
  error(message('MATLAB:ghostscript:ProblemConvertingPostScriptSystemReturnedError', num2str(s), r)) 
elseif s
...
if ( fid == -1 )
    error(message('MATLAB:ghostscript:CouldNotCreate', pj.GhostName));
else
    fclose( fid );
end
...
% LocalWords:  dlaserjet dljetplus dljet dpxlmono ddeskjet ddjet Deskjet
% LocalWords:  dcdjmono dpaintjet dpjxl dpjetxl dbj dbjc BJC depson Epson
% LocalWords:  depsonc LQ deps dibmpro Proprinter dcdjcolor Steinberg dcdj ddnj
% LocalWords:  dbmpmono dbmp dpcxmono PCX dpcx EGA dpbm dpbmraw dpgm Graymap
% LocalWords:  dpgmraw dppm Pixmap dppmraw dpdf rsp ghostdir ps
