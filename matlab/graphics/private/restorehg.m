...
if ~isequal(size(h), [1 1]) || ~isfigure( h )
    error(message('MATLAB:restorehg:InvalidHandle'))
end
...
% give whatever other pending updates there might be a chance
% waited until after (possibly) restoring renderer above, to make sure 
% we draw with the "right" one... 
drawnow; 
...
% LocalWords:  pj PREPAREHG RESTOREUI mfile erasemode IM renderermode Fcn's
% LocalWords:  fontunits backingstore XOR'ed
