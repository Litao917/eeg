...
if pt.DebugMode
    disp(sprintf(getString(message('MATLAB:uistring:ptpreparehg:PreparingFigure', num2str(double(h))))))
    pt
end
...
if pt.AxesFreezeTicks
    ...
    pt.tickState.values = LocalGetValues(theAxes, {'XTickMode','YTickMode','ZTickMode'} );
    LocalSetValues( pt.tickState.handles, {'XTickMode','YTickMode','ZTickMode'}, {'manual','manual','manual'})
end

% Output Axes with same tick LIMITS as on screen
if pt.AxesFreezeLimits
    pt.limState.handles = theAxes; 
    pt.limState.values = LocalGetValues(theAxes, {'XLimMode','YLimMode','ZLimMode'} );
    LocalSetValues( pt.limState.handles, {'XLimMode','YLimMode','ZLimMode'}, {'manual','manual','manual'})
end
...
if (~isempty(oldvalue))
  ...
  LocalSetValues(inArray,{prop},newvalue);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function values = LocalGetValues(inArray, props)
% Checks to make sure a property exists then return the value or empty if
% it does not exist
values = cell(length(inArray),length(props));
for i=1:length(inArray)
    for j=1:length(props)
        if isprop(inArray(i), props{j})
            values{i,j} = get(inArray(i), props{j});
        else
            values{i,j} = [];
        end
    end
