...
% This app data is used by scribe. It needs to be set on the current root
...
if ~isempty(s.children)
    ...
    if nlegs>0
      ...
      for k = 1:length(legs)
        ...
        if legs(k).phindex == 0
          warning(message('MATLAB:hgload:InvalidLegendPeer'));
          oklegs(k) = false;
        end
      end
      legs = legs(oklegs);
...
for k=1:length(axlist)
  ...
  safeRemoveAppData(ax,'SubplotDeleteListenersManager');
