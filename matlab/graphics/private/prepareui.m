...
if ~isequal(size(Fig), [1 1]) || (~isempty(Fig) && ~isfigure(Fig))
    error(message('MATLAB:prepareui:InvalidFigureHandle'))
end
...
% save current axes, if any, for later restoration
pj.UIData.CurrentAxes = get(Fig, 'CurrentAxes'); 
...
for lp=1:length(Frame)
    if ~isempty(CapturedImages{lp})
        ...
        warning(message('MATLAB:print:UIcontrolScreenCaptureFailed'));
...
%A little fudging is required since pop-up menus do not obey their
...
% LocalWords:  uicontrols pj PREPAREHG RESTOREUI hpgl hgjavacomponent awt
% LocalWords:  windowstyle popupmenu restoreui minrect GL YTick YDir UIcontrol
% LocalWords:  capturedimages lp seg v's RECTAROUNDCONTROL
