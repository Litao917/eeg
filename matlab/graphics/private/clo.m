...
if do_reset
    if ishghandle(obj)
        if graphicsversion(obj,'handlegraphics')
            ...
            kids = allchild(obj);
kids_to_delete = intersect(findall(kids, 'serializable', 'on'), kids,'legacy');
        end
    else
...
if any(ishghandle(obj)) && ~graphicsversion(obj,'handlegraphics')
    hlen = length(hsave);
    for i=1:hlen
    ...
    kids_to_delete = setdiff(kids_to_delete, hsave,'legacy');
end
