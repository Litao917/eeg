...
try
  DlgName = getString(message('MATLAB:printpreview:PrintPreviewDlg', LGetFigName(fig)));
  set(dlg,'Name',DlgName);
  dlgPos = LInitializeDialogPos(fig,dlg);
  % pushbuttons
  buttons(1) = uicontrol(dlg,'String',getString(message('MATLAB:printpreview:PrintButton')),'Callback',{@LPrintCallback, dlg});
  buttons(2) = uicontrol(dlg,'String',getString(message('MATLAB:printpreview:PageSetupButton')),'Callback',{@LPageSetupCallback, dlg});
  buttons(3) = uicontrol(dlg,'String',getString(message('MATLAB:printpreview:HeaderButton')),'Callback',{@LHeaderCallback, dlg});
  buttons(4) = uicontrol(dlg,'String',getString(message('MATLAB:printpreview:RefreshButton')),'Callback',{@LRefreshCallback, dlg});
  buttons(5) = uicontrol(dlg,'String',getString(message('MATLAB:printpreview:CloseButton')),'Callback',{@LCloseCallback, dlg});
...
try
  ...
  try
    ...
    imap = flip(hardcopy(fig,hcform,DPISwitch),1);
  catch ex
    fRenderError = 1;
  end
...
while imsize > 8000000
  ...
  imsize = prod(res * paperPos(3:4));
  if ~alreadywarned
    warning(message('MATLAB:printpreview:ConserveMemory'))
    alreadywarned = true;
  end
end
