function varargout = oldPrintPreview(varargin)
% This undocumented helper function is for internal use.

%Display preview of figure to be printed. This will be called from
%printpreview if useOriginalHGPrinting returns true. 
...
if isempty(figPreview) || ~ishghandle(figPreview)
    ...
    figPreview = figure('Name',getString(message('MATLAB:uistring:printpreview:TitlePrintPreview')),...
                        'IntegerHandle','off',...
...
%    end
    ...
    r = awtinvoke(panel, 'addTabbedPanel(Ljava/lang/String;Ljava/lang/String;Lcom/mathworks/mwswing/MJPanel;)', getString(message('MATLAB:uistring:printpreview:LayoutTab')),'Layout', p);  %#ok
    %panel.addTabbedPanel(getString(message('MATLAB:uistrings:printpreview:LayoutTab')), com.mathworks.page.export.PreviewTabLayout(model));
    ...
    r = awtinvoke(panel, 'addTabbedPanel(Ljava/lang/String;Ljava/lang/String;Lcom/mathworks/mwswing/MJPanel;)', getString(message('MATLAB:uistring:printpreview:LinesTextTab')),'Lines/Text', p);  %#ok
    %panel.addTabbedPanel(getString(message('MATLAB:uistrings:printpreview:LinesTextTab')), com.mathworks.page.export.PreviewTabLines(model,fontList,defaultFontName));
    p = awtcreate('com.mathworks.page.export.PreviewTabColor', '(Lcom.mathworks.page.export.PrintExportSettings;)', model);
    r = awtinvoke(panel, 'addTabbedPanel(Ljava/lang/String;Ljava/lang/String;Lcom/mathworks/mwswing/MJPanel;)', getString(message('MATLAB:uistring:printpreview:ColorTab')),'Color', p);  %#ok
    %panel.addTabbedPanel(getString(message('MATLAB:uistrings:printpreview:ColorTab')), com.mathworks.page.export.PreviewTabColor(model));
    p = awtcreate('com.mathworks.page.export.PreviewTabMisc', '(Lcom.mathworks.page.export.PrintExportSettings;)', model);
    r = awtinvoke(panel, 'addTabbedPanel(Ljava/lang/String;Ljava/lang/String;Lcom/mathworks/mwswing/MJPanel;)', getString(message('MATLAB:uistring:printpreview:AdvancedTab')),'Advanced', p);  %#ok
    %panel.addTabbedPanel(getString(message('MATLAB:uistrings:printpreview:AdvancedTab')), com.mathworks.page.export.PreviewTabMisc(model));
    ...
    if useOriginalHGPrinting(fig)
        ...
        %%%%%%% printing.
        set(figPreview,'ResizeFcn',{@onResize, fig})        
    end
...
if useOriginalHGPrinting(fig) 
    % Get CData for Original HG Printing 
    pj = printjob(fig);
    pj.DriverColor = defaultprtcolor(fig);
    pj.DriverClass = 'IM';
    pj.Driver = '-dzbuffer';
    pj.DriverColorSet = 0;
    pj.DPI = ceil(res);
    ...
    cdata = flip(hardcopy(fig,hcform,DPISwitch),1);
   ...
   % Get CData for New HG Printing
   figRenderer = get(fig,'renderer');
   if strcmpi(figRenderer,'none')
       figRenderer = 'opengl';
   end
   figRenderer = ['-d' figRenderer];
   
   [msgstr, msgid] = lastwarn;
   hardcopyWarning = warning('off','MATLAB:hardcopy:DeprecatedHardcopyFunction');
   try
      cdata = flip(hardcopy(fig, figRenderer, DPISwitch),1);
   catch ex
       warning(hardcopyWarning.state, hardcopyWarning.identifier);
       rethrow(ex)
   end
   warning(hardcopyWarning.state, hardcopyWarning.identifier);
