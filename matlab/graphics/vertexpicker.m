...
if ~graphicsversion(obj,'handlegraphics')
    try 
        [pout,vout,viout,pfactor,facevout] = vertexpickerHGUsingMATLABClasses(obj,target,mode);
    catch me
        throw(me)
    end
    varargout = {pout,vout,viout,pfactor,facevout};
...
% Performance optimization for 2-D lines 
...
% Apply the axis scale for x, y, and z
vert(:,1) = local_ApplyLogScaleData(get(ax,'XScale'), get(ax,'XLim'), vert(:,1));
vert(:,2) = local_ApplyLogScaleData(get(ax,'YScale'), get(ax,'YLim'), vert(:,2));
vert(:,3) = local_ApplyLogScaleData(get(ax,'ZScale'), get(ax,'ZLim'), vert(:,3));
...
%--------------------------------------------------------%
function vert = local_ApplyLogScaleData(scale, lims, vert)
% Apply an axis scale to a vector of data.   This code is based on HG's
% gs_data3matrix_to_pixel internal c-function.
if strcmp(scale,'log')
    if any(lims > 0)
        % On a positive log-scale plot, negative values do not render.
        IsRendered = vert>0;
        vert(IsRendered) = log(vert(IsRendered));
        vert(~IsRendered) = NaN;
    else
        % On a negative log-scale plot, positive values do not render.
        IsRendered = vert<0;
        vert(IsRendered) = -log(-vert(IsRendered));
        vert(~IsRendered) = NaN;
    end
...
%--------------------------------------------------------%
function [pout, vout, viout] = local_ImagePicker(hImage,target,~)
...
if ~isempty(ind)
    ...
    %each other. The interpolation should handle this smoothly. Loop through
...
if ~isempty(get(hLine,'zdata'))
    if strcmpi(get(hAx,'ZScale'),'linear')
        ...
        zpos = zsign*exp(tmin * log(zsign*zdata(imin)) + (1-tmin) * log(zsign*zdata(imin+1)));
    end
...
% polygon is intersected if it traverses an odd number of polygon 
% edges along any positive axis. 

% Output variables
pout=[];vout=[];viout=[];pfactor=[];facevout=[];faceiout=[];xdist=[]; 

% Fix problems with NaNs in the faces definition
