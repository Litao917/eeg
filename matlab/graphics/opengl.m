    ...
    switch(lower(mode)),
      ...
      case 'software',
        if strncmp(computer,'MAC',3)
            warning(message('MATLAB:opengl:switchToSoftwareMacNotSupported'));
        elseif isunix
            if feature('OpenGLLoadStatus') == 0
                feature('UseMesaSoftwareOpenGL',1)
            else
                warning(message('MATLAB:opengl:switchToSoftwareUnixNotSupported'));
            end
        else % windows
      ...
      case 'hardware'
        if strncmp(computer,'MAC',3)
            warning(message('MATLAB:opengl:switchToHardwareMacNotSupported'));
        elseif isunix
            if feature('OpenGLLoadStatus') == 0
                feature('UseMesaSoftwareOpenGL',0)
            else
                warning(message('MATLAB:opengl:switchToHardwareUnixNotSupported'));
            end
        else % windows
            feature('OpenGLLoadStatus',1)
            if feature('OpenGLLoadStatus') == 0
      ...
      case 'info',
        try
            ...
            if feature('OpenGLLoadStatus') == 0
                ...
                warning(message('MATLAB:opengl:loadStatus'))
                if(nargout)
                    str = 0;
                end        
            else
            ...
            if(nargout==0)
                warning(message('MATLAB:opengl:infoQuery'))
      ...
      case 'data'
        ...
        try
            ...
            warning(message('MATLAB:opengl:dataQuery'))
            disp(ex.getReport('basic'));
        end
      case 'verbose'
      ...
      otherwise
        if ~strncmpi(mode,'opengl',6)
            ...
            try
                ...
                error(message('MATLAB:opengl:invalidBugWorkaround', mode))
            end
        end
    end
end
