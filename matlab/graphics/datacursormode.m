...
if ~graphicsversion(fig, 'handlegraphics')
    try
        if nargout == 0
            datacursormodeHGUsingMATLABClasses(fig, action);
        else
            retval = datacursormodeHGUsingMATLABClasses(fig, action);
        end
    catch me
        throw(me)
    end
...
DEFAULT_STR = getString(message('MATLAB:uistring:datacursor:LabelMouseClickOnPlottedData'));
...
if isempty(hFrame) || ~ishghandle(hFrame)
   ...
   tSize = hTool.figpanel(hFrame,'String',DEFAULT_STR);
...
if ~isempty(hDatatip) && ishghandle(hDatatip)  && ...
    ...
    if ~isempty(hTarget) && ishghandle(hTarget)
        ...
        if isempty(title_name)
            title_name = ...
                [getString(message('MATLAB:uistring:datacursor:Type'))...
                ' '...
                get(classhandle(handle(hTarget)),'Name')];
        end
    end
end
...
if ~isempty(hPanel) && ishghandle(hPanel)
        ...
        tSize = hTool.figpanel(hPanel,'String',str);
        ...
        hTool.figpanel(hPanel,'Title', title_name);
end
