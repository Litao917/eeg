...
if nargin
    ...
    if nargin == 1
        if ~any(ishandle_valid(obj)) || (~ishghandle(obj,'figure') && ~ishghandle(obj,'axes'))
...
if ~graphicsversion(obj,'handlegraphics') % Work around G621525
    com.mathworks.page.cmapeditor.CMEditFrame.sendJavaWindowFocusEventsToMatlab(true);
end
cme = CMEditor;
set_cmapeditor(cme);

% Set HG Version in Colormap Editor
cme.setHGUsingMATLABClasses(~graphicsversion(obj,'handlegraphics'));
cme.init;
...
fig = get(0, 'CurrentFigure');
if handle(fig)==handle(oldfig)
    % Nothing to do here, since it's the same figure
    return;
end
set_current_object(fig);
set_current_figure(fig,oldfig,oldax);
...
if ~graphicsversion(oldfig,'handlegraphics')
...
if isempty(ax) || ...
   (graphicsversion(oldfig,'handlegraphics') && strcmp(get(ax,'BeingDeleted'),'on')) || ...
   (~graphicsversion(oldfig,'handlegraphics') && ~isvalid(ax))
    return;
end
...
% Set HG Version in Colormap Editor
cme.setHGUsingMATLABClasses(~graphicsversion(fig,'handlegraphics'));
...
if any(ishandle_valid(fig))
    ...
    if ~graphicsversion(fig,'handlegraphics')
        ...
        fl.numberTitle = handle.listener(fig, cls.findprop('NumberTitle'), ...
                  'PropertyPostSet',@(es,ed) updateTitle(es,ed));
        fl.nameSet = handle.listener(fig, cls.findprop('Name'), ...
                  'PropertyPostSet',@(es,ed) updateTitle(es,ed));
    end
    setappdata(fig,'CMEditFigListeners',fl);
end
...
if ~graphicsversion(fig,'handlegraphics')
    ...
    figureNumber = num2str(double(fig));
end
title = '';
separator = '';
...
if ishghandle(obj,'axes') && ~graphicsversion(fig,'handlegraphics')
    
        if ~isempty(obj.TitleHandle) && ~isempty(obj.Title.String)
            title = [title ': ' obj.Title.String];
        else
            title = [title ': ' getString(message('MATLAB:uistring:colormapeditor:UntitledAxes'))];
        end
