...
if ~isequal(size(ny),[m,n])
    error(message('MATLAB:specular:NySizeMustEqualNx')); 
end
if ~isequal(size(nz),[m,n])
    error(message('MATLAB:specular:NzSizeMustEqualNx')); 
end
...
elseif length(s)==3 
  ...
  error(message('MATLAB:specular:SpecifyInput'));
end
...
elseif length(v)==3 
  ...
  error(message('MATLAB:specular:MustSpecifyV'));
end
