...
if nargin > 0
    ...
    hMapObject = getAlphaMapContainer(param1);
    ...
    if ischar(param1)
        switch param1
            ...
            case 'default'
                ...
                try
                    gv = graphicsversion(hMapObject,'handlegraphics');
                catch err
                    gv = true;
                end
...
% Return the object that contains the alphamap property
function alphamapContainer = getAlphaMapContainer(obj)
alphamapContainer = [];

try
    gv = graphicsversion(obj,'handlegraphics');
catch err
    gv = true;
end
                
if(~gv)
    alphamapContainer = getMapContainerHGUsingMATLABClasses(obj);
else
    if ishghandle(obj,'figure')
        alphamapContainer = obj;
    elseif ishghandle(obj,'axes')
        alphamapContainer = ancestor(obj,'figure');
    end
end
