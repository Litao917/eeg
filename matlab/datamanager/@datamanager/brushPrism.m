    ...
    try %#ok<TRYNC> % xdata,ydata may be out of sync in an HG error state
        ...
        if ~isempty(lastregion)
            ...
            if linkedPlot && isa(gObjH,'graph3d.surfaceplot')
                ...
                I1(I) = true;
                Irows1 = any(I1,2);
                Iexpanded = logical(Irows1*ones(1,size(I1,2)));
                Icontracted = [];
            else
                Iexpanded = I;
                Icontracted = [];
...
for row=1:3
    ...
    if ~isempty(varName)
        ...
        if row<=2 && ~isempty(Iextend) && isvector(Igraphic) && ...
                ~isvector(Iextend)
            if row==2 % YDataSource - 1st dimension of ZData
                IextendLoop = any(Iextend,2);
        ...
        if row<=2 && ~isempty(Icontract) && isvector(Igraphic) && ...
                ~isvector(Icontract) 
            if row==2 % YDataSource - 1st dimension of ZData
                IcontractLoop = any(Icontract,2);
            else % XDataSource - 2nd dimension of ZData
                continue; 
            end
        else
            IcontractLoop = Icontract;
        end
        
        if extendMode
            Igraphic(IextendLoop) = ~Igraphic(IextendLoop);
            Igraphic(IcontractLoop) = ~Igraphic(IcontractLoop);            
        else
            Igraphic(IextendLoop) = true;
            Igraphic(IcontractLoop) = false;
        end
