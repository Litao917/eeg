...
for k=1:length(linkManager.Figures)
    ...
    if ~isempty(linkedGraphics)
        ...
        Iunbrushable = [];
        if ~graphicsversion(linkManager.Figures(k).Figure,'handlegraphics')
        ...
        if graphicsversion(linkManager.Figures(k).Figure,'handlegraphics') % Data Listeners not needed for hg2 brushing annotations
...
if (graphicsversion(fig,'handlegraphics') && isa(gObj,'graph3d.surfaceplot')) || ...
  (~graphicsversion(fig,'handlegraphics') && isa(gObj,datamanager.getChartingObjectTypesUsingMATLABClasses('Surface'))) 
...
elseif (graphicsversion(fig,'handlegraphics') && (isa(gObj,'specgraph.barseries') || isa(gObj,'specgraph.areaseries'))) || ...
      (~graphicsversion(fig,'handlegraphics') && (isa(gObj,datamanager.getChartingObjectTypesUsingMATLABClasses('Bar')) || ...
      isa(gObj,datamanager.getChartingObjectTypesUsingMATLABClasses('Area'))))
