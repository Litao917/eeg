...
if isempty(hMode)
    ...
    if isempty(hFig_.findprop('BrushStyleMap'))
        if ~graphicsversion(hFig_,'handlegraphics')
            brushStyleMapProp = addprop(hFig_,'BrushStyleMap');
            brushStyleMapProp.Hidden = true;
        else
            brushStyleMapProp = schema.prop(hFig_,'BrushStyleMap','MATLAB array');
    ...
    if graphicsversion(hFig,'handlegraphics') && isempty(hMode.ModeStateData.brushObjects)
